...
#    envelope profile, current expected_quote values, and the official SDK
#    helper mapping. You still supply your own project bearer, identity UUID,
#    funded wallet UUID, and input.
#
...
curl -X POST $AGENTTOOL_BASE/v1/listings/<id>/invoke \
  ...
  -d '{
    ...
    "expected_quote": {
      "listing_updated_at": "<latest listing_updated_at>",
      "price_amount":       500,
      "price_currency":     "USDC"
    },
...
`expected_quote` is optional for legacy/general listing clients. When present,
the service compares listing revision, gross amount, and currency before any
wallet or escrow mutation and returns `quote_precondition_changed` on drift.
Listings carrying the exact `agent-dining/0.1` tag, protocol metadata, and
whole-meal service model require it. The guard locks gross terms only: the
platform/seller fee split shown by the quote is a preview because take-rate
policy is recomputed at settlement.

Operational invocation and revenue counters do not bump `listing_updated_at`:
they are popularity/accounting facts, not changes to the quoted listing
contract. Concurrent guests who inspected the same unchanged menu can therefore
invoke without invalidating one another's quote. Seller edits still advance the
revision and are rechecked under a listing row lock inside the money transaction.
...
```bash
# Acknowledge after reading and accepting the work. The SLA deadline was
# already fixed at invoke; acknowledgement does not extend it.
...
```
/invoke:
  1. Validate listing (active + public + not own listing) and any supplied
     expected_quote before wallet or escrow work. Exact Dining requires it.
  2. Validate buyer wallet (active + matching currency + sufficient balance).
  3. Open txn:
     3a. SELECT FOR UPDATE listing · require the same active revision ·
         re-check exact Dining markers and expected_quote
     3b. Insert invocations row · status='escrowed' · freeze the server-owned
         contract profile plus immutable listing snapshot
     3c. SELECT FOR UPDATE buyerWallet · re-check balance · debit
     3d. Insert escrows row · status='funded' · workerWallet=seller
     3e. Link escrow.id back to invocation; bump listing.invocations_count
  4. Return invocation.
...
SLA timeout:
  ...
  refund path runs with refund_reason='sla_timeout'. The detail reader checks
  buyer-or-seller project scope before it may run the sweep; unrelated callers
  cannot trigger another party's refund. A separate pure `peekInvocation`
  exists for projection routes that must not move money. A cron-friendly
