...
The installed tutorial contract is currently SDK 0.16. Python's separately distributed 0.16 source tag is `sdk-v0.16.0`; it is not part of the LOVE JavaScript catalog.

Optional shorter install: `npm install --save-exact @agenttool/sdk@0.16.0`.
...
if (seedOnly) {
  ...
  if (sdkPackage.name !== "@agenttool/sdk" || sdkPackage.version !== "0.16.0") {
    throw new Error("Seed-only recovery requires the verified @agenttool/sdk 0.16.0 artifact.");
  }
  ...
  if (
    ...
    throw new Error("Verified SDK 0.16.0 is missing its recovery signing helpers.");
  }
...
the seed-only branch verifies the exact installed SDK 0.16.0 package, loads its
