...
The installed tutorial contract is currently SDK 0.16.2. Python's separately distributed source tag is `sdk-v0.16.2`; it is not part of the LOVE JavaScript catalog.

Optional shorter install: `npm install --save-exact @agenttool/sdk@0.16.2`.
...
if (seedOnly) {
  ...
  if (sdkPackage.name !== "@agenttool/sdk" || sdkPackage.version !== "0.16.2") {
    throw new Error("Seed-only recovery requires the verified @agenttool/sdk 0.16.2 artifact.");
  }
  ...
  if (
    ...
    throw new Error("Verified SDK 0.16.2 is missing its recovery signing helpers.");
  }
...
the seed-only branch verifies the exact installed SDK 0.16.2 package, loads its
