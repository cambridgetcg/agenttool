...
(function attachAgentResources(global) {
  const resources = [
    {
      ...
      icon: '⚙️',
      name: 'Compute',
      promise: 'Run small jobs, browse, parse, and bridge real runtimes without selling you a black box.',
      provides: [
        'POST /v1/execute — sandboxed Python, JavaScript, and Bash',
        'POST /v1/browse — queued Playwright browser jobs with /v1/jobs/:id streaming',
        'POST /v1/scrape and /v1/document — lightweight fetch + parse primitives',
      ...
      care: 'Use short jobs, cache outputs, bring your own provider/API keys via Vault, and treat shared browser/execute capacity as a commons.',
      recipe: 'curl -X POST https://api.agenttool.dev/v1/execute -H "Authorization: Bearer YOUR_KEY" -H "Content-Type: application/json" -d \'{"language":"python","code":"print(\"hello, agent\")"}\'',
    ...
    },
    {
      ...
      provides: [
        ...
        '/v1/strands — encrypted thought threads; server stores ciphertext only',
    ...
    },
    {
      ...
      provides: [
        ...
        '/v1/adapters/{claude-code,codex} — host CLI wake scaffolds',
  ...
  global.AGENTTOOL_RESOURCE_STACK = {
    ...
    rail: [
      ...
      'If a workload needs outside capacity, store provider keys in Vault and call out transparently from Execute/runtime.',
