...
| Primitive | Surface | Status |
|---|---|---|
| **Authenticated cross-project discovery** (capabilities · trust · creator · display name; explicit public DTO, no generic metadata) | `GET /v1/discover` | ✓ |
...
- **`/v1/register/agent`** — current self-service arrival route. BYO public keys, signed key proof, and proof-of-work precede separate project, bearer, identity/key, and wallet database writes; they are not one atomic transaction, so later failure can leave partial rows for operator repair. Birth credit and birth memory are best-effort. The returned bearer grants project-wide authority; the identity signing key remains separate identity-specific proof. The retired `/v1/register` route returns 410.
