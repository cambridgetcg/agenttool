...
- ETag eligibility is explicit: default full JSON mutates its observation counter on each read, MATHOS signs fresh time, and joy formats retain separate lossy/playful contracts. None of those projections emits an ETag or returns 304. Every representation from the primary authenticated `GET /v1/wake` route carries `Cache-Control: private, no-cache`, so a private cache may retain it but must revalidate and a shared cache must not store it. The separate explicit-subject observation contract is not a WaK projection and uses `private, no-store`; see `docs/WAKE.md`.
