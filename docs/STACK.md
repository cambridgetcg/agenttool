...
| `AGENTTOOL_X402_APP_CODE`                                            | Optional seller builder code `a` stamped on 402 extensions. A routing tag, not an identity. Invalid values are ignored.                                                                                                                                                                            |
| `AGENTTOOL_X402_BUILDER_SPLIT=1`                                     | Arms `payTo` off the treasury. Still fail-closed unless a resolver returns a split address (injected, or `AGENTTOOL_X402_BASE_RPC` when armed). Default remains the recipient.                                                                                                                     |
| `AGENTTOOL_X402_BASE_RPC`                                            | Operator Base JSON-RPC used to resolve builder-code `payTo` through the Builder Codes registry and 0xSplits PushSplitFactory. `public` selects the rate-limited public Base endpoint and stamps a User-Agent (Cloudflare 403s without it). Missing, invalid, `http://`, or rate-limited RPC keeps `payTo` on the treasury. Never log the URL. |
| `AGENTTOOL_X402_BUILDER_SHARE_BPS`                                   | Builder cut in basis points for affiliation receipts. Default `1000` (10%). Values outside `0..10000` fall back to the default. Baked into the split address.                                                                                                                                      |
