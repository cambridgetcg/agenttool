...
Separate real HTTP journeys use the local TypeScript and Python 0.22.1 SDK sources against this fixture. Each writes and fsyncs an owner-only mnemonic before ordinary 18-bit bootstrap, then stores, reads and text-searches memory and reads the selected wake. A fresh process reloads the retained root and bearer, recalls the same memory, performs signed lookup and recovery, verifies the replacement bearer, explicitly revokes the old bearer and proves its rejection. Recovery uses exported SDK signing helpers plus explicit HTTP/authority orchestration; neither SDK supplies a one-call recovery client. These four phases complete 32 real HTTP requests without outbound API fetches or application errors. Standard Python urllib also passes a separate memory-read smoke. Loopback success does not resolve the observed production Cloudflare rejection.
