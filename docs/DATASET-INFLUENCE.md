...
**Status:** public-ready repository source candidate `@agenttool/dataset-influence@0.1.0-dev.0`. The Hugging Face tree is deterministic and names the intended identifier `Yu-and-Ai/agenttool-dataset-influence`, but these repository bytes do not establish a Hub repository or publication. No hosted route, identity write, wallet action, marketplace settlement, model run, training run, dataset/weight download, provider call, or production deployment is included.
