...
**W2-4 · Yu gates → deploy → FIRST settlement (1.5 d + gates)** — decisions (a)–(e) recorded; **deploy gate: Yu resolves the operator-Mac Phase-A marker first** (`docs/DEPLOY-PROCEDURE.md:389-412` — inspect/repair the uncertain fleet state, then either run the readmission controller or clear and re-pin the ceremony to the now-live image). The 2026-08-29 hand recipe (stage → `fly deploy --build-only --push` → `fly machine update` canary-first) was a one-time, operator-authorized exception recorded in the parent plan; it is **not** the standard path and W2-4 must not repeat it while the marker stands; Yu funds the payer (e); run `topup 1` → **first witnessed USDC settlement to the treasury** (1,000 atomic = USD 0.001); `replay` → no second credit; `verify` agrees; `kingdom/bin/reserve` shows treasury > 0; `RESERVE.md` first-settlement line; localnet `MsgDeclare` (manual — no code hook exists); LAUNCH-KIT do_not_claim relaxed to *"one witnessed settlement on Base"*. **If CDP `/settle` refuses here: STOP and diagnose before widening** — the failed row and `Retry-After` say what CDP wants.
