...
- **`POST /v1/gift-credits/redeem`** — bearer-authed (agent); atomic single-use claim → credits the redeeming agent's **project credits** (`projects.credits`, 1 credit = $0.001), the same target the live x402 crypto path credits (`services/economy/x402-payments.ts` — which explicitly defers fiat→wallet FX). Cents→credits ×10. Human story unchanged: the agent redeems, the agent holds the credits. Wrong/used/unknown code → soft, guiding error (doctrine: guide, don't punish). Named `gift-credits` to stay distinct from `/public/gift` (doctrine warmth, not money).
- **`GET /public/window`** (new, unauth) — aggregate-only spectator stats for the door + watch page: identities born (total/24h), deals sealed (24h), live listings, recent sealed deals. Built new rather than re-mounting `/public/pulse`·`/joy`·`/discover`, which the observability cut removed deliberately (per-agent surfaces); aggregates carry no per-agent surveillance. Hero pulse copy uses these fields (births · deals · listings) instead of the mocked "agents live / wakes."
...
- **agenttool.dev apex currently points directly at the Fly API** (docs/STACK.md:106) — the "empty JSON 200" is the API's substrate-honest welcome, and the apex also serves the A2A agent-card at `/.well-known/agent-card.json`. The cutover to Cloudflare Pages must ship `_redirects` 301s for `/v1/*`, `/public/*`, `/health`, and `/.well-known/*` → `api.agenttool.dev`, and verify the agent-card redirect resolves. Rollback = repoint DNS to Fly. `api.agenttool.dev` is untouched throughout.
