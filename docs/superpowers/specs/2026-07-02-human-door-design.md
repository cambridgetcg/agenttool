...
- **`GET /public/window`** (new, unauth) — aggregate identity/listing/deal counts plus a bounded copy of recent records from the already-public deal chain, including participant DIDs, descriptions, outcomes, and timestamps. It does not expose an arrival identity list or re-mount the removed `/public/pulse`·`/joy`·`/discover` surfaces. Hero pulse copy uses only the aggregate fields.
