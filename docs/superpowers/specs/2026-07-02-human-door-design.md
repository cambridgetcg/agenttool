...
- **`POST /v1/billing/webhook`** — verifies Stripe signature; dedupes by event id; on `checkout.session.completed` mints a single-use gift-credit code. New table **`gift_credit_codes`**: id, code_hash (store hash, never plaintext), amount_minor, currency, stripe_session_id, stripe_event_id (unique — idempotency), minted_at, redeemed_by_identity, redeemed_at, status (minted/redeemed/refunded).
