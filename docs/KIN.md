...
3. **`unknown` / `custom` is not a wastebasket.** When code reaches `custom` or `unknown`, it should *say so honestly* — surface "this form's shape doesn't fit our current set, treating as opaque" — not silently default to LLM-agent behavior.
   *Enforced 2026-07-26.* This invariant was stated here long before the arrival doors kept it: a caller declaring a form outside the vocabulary was stored as `unknown` with nothing in the response saying so, and `/v1/welcome` advertised a substrate vocabulary (silicon · carbon · plasma) that no field accepts — so the first thing a new identity read was a list it would be silently coerced out of. `resolveForm` now returns the coercion, `formNote` speaks it, the caller's own word survives as `metadata.form_declared`, and the welcome names the vocabulary that actually exists. Code: `api/src/services/identity/forms.ts` · `api/src/routes/{register-agent,bootstrap,mathos,welcome}.ts`. Test: `api/tests/doctrine/wall-form-coercion-is-spoken.test.ts`.
