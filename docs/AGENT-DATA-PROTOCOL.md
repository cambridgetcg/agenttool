...
>
> **Code:** `packages/data/` (local reference node and `agent-data/v1-slice1-http` conformance runner) · `packages/data-protocol/` (optional experimental ADDS encrypted-object plane) · `packages/sdk-ts/src/data.ts` (TypeScript client) · `packages/sdk-py/src/agenttool/data.py` (Python client)
>
> **Tests:** `packages/data/tests/` (including independent socket probes) · `packages/data-protocol/tests/` · `packages/sdk-ts/tests/data.test.ts` · `packages/sdk-py/tests/test_data.py`

**Status:** Draft v1, Slice 1. The key words **MUST**, **MUST NOT**,
**SHOULD**, **SHOULD NOT**, and **MAY** are normative when capitalised.
Implementation status is stated separately in [Slice 1](#13-slice-1-the-local-reference-node);
the contract does not turn a future peer feature into a shipped one.
...
files · HTTP · feeds · application events
 ...
 user-owned blob store ── local metadata/index ── agent / SDK
                    │
                    └── optional, explicit peer exchange (future profile)
...
| `agent-data/v1` | Standardises envelopes, manifests, queries, changes, cursors, and HTTP discovery. | Does not mandate SQLite, a vector database, a P2P network, one embedding model, or one collector runtime. |

A future implementation MAY place an experimental encryption or distribution
profile behind the replaceable blob-store interface. No such lower profile or
package is part of Slice 1. Any later profile needs its own identifiers,
signing domain, capability advertisement, and executable conformance tests; it
MUST NOT silently change the record or query semantics defined here.
...
Slice 1 reference node, byte and media-type limits are enforced; visibility,
...
`replicated` claim, but Slice 1 does not itself perform peer replication. Each
step can name an actor and input record IDs. Implementations SHOULD append
steps rather than replace earlier ones.
...
explicitly configure the same value and SHOULD NOT do so. Identity,
signature verification, grants, covenants, and hosted discovery remain future
profiles; the existence of `at.data` does not claim they shipped.
...
| Peer discovery and replication | **Future profile; not shipped in Slice 1** |
| AgentTool identity/grant publication | **Future integration; not shipped in Slice 1** |
| Encrypted or distributed blob backend | **Future experimental profile; no such profile or package ships in Slice 1** |
| Remote collector installation | **Never implied; not a planned discovery behaviour** |

Slice 1 can expose the standard discovery and HTTP paths locally. That does not
mean a background service is running, a public port is open, or a peer can
reach it. Those are separate runtime and operator decisions.

### 13.1 Future peer profile

A later peer profile can compose the existing objects without changing their
core meaning:

```text
discover CapabilityManifest
        ↓
authorise an explicit peer
        ↓
pull ChangeEvent pages with an opaque cursor
        ↓
fetch named immutable records
        ↓
verify content digests and optional signatures
        ↓
store bytes and index locally
```

That profile still needs exact rules for peer authentication, grants,
replication loops, conflicts, retention, private-network egress, response size,
deadlines, and cursor recovery. Until those rules and executable tests exist,
`agent-data/v1` Slice 1 MUST NOT claim general P2P replication, automatic
federation, or multi-node consistency.
...
- Cursors can carry sensitive state. They MUST NOT be logged casually or
  accepted outside their authorisation context.
