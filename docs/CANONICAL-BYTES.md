...
Used in: `services/identity/delegation.ts` (`canonicalDelegationBytesV2`) · `POST /v1/delegations` · `GET /v1/delegations/:id/verify` (read-time re-verification, reporting which domain stood up). Client signers: `packages/sdk-ts/src/identity.ts` (`signDelegation`) · `packages/sdk-py/src/agenttool/identity.py` (`sign_delegation`). Pinned by the shared fixture (`agenttool-delegation/v2`, 17 cases across all three implementations) and by `api/tests/delegation-canonical-bytes.test.ts` · `packages/sdk-ts/tests/delegation-signing.test.ts` · `packages/sdk-py/tests/test_delegation_signing.py`.
...
Used in: `services/economy/crypto/address-claim.ts` · `POST /v1/wallets/:id/addresses`. Pinned by `api/tests/wallet-address-claim.test.ts` and by the shared fixture (`wallet-address-claim/v1`, 8 cases across all three implementations). Doctrine: `docs/CRYPTO-PAYMENT.md` · `docs/IDENTITY-SEED.md` (purpose=5, `m/44'/169'/5'/<wallet-index>'`).
...
- `api/tests/delegation-canonical-bytes.test.ts`, `packages/sdk-ts/tests/delegation-signing.test.ts`, and `packages/sdk-py/tests/test_delegation_signing.py` — `agenttool-delegation/v2` (and v1 compatibility)
- `api/tests/wallet-address-claim.test.ts`, `packages/sdk-ts/tests/wallet-address-claim.test.ts`, and `packages/sdk-py/tests/test_wallet_address_claim.py` — `wallet-address-claim/v1`
