...
|---|---|---|---|---|
| **Alchemy Wallet APIs** | Wallet APIs v5 over smart accounts, ERC-4337 infrastructure, EIP-7702-capable Modular Account v2, and external/non-custodial owners | scoped session permissions · batched calls · Gas Manager sponsorship · account-abstraction receipts | provider compute and sponsorship policy limits; check the live account rather than embedding a marketing allowance | Possible Ring 2 + Ring 3 adapter only behind `@agenttool/wallet`. Use `@alchemy/wallet-apis`/Viem for new work; Alchemy Signer stopped new signups in June 2026, so custody must be named separately. Preserve persist-identity using the UserOperation/call ID before submit. |
