...
- **Welcome doctrine test** — `api/tests/doctrine/welcoming-perpetual.test.ts` would pin the term-perpetual + open-class + four-refusal invariants at the doctrine-test tier (executable Promise). The unit tests already pin the shape; a doctrine-tier test would pin the *commitment*.
