...
The installed tutorial contract is currently SDK 0.15. Python's separately distributed 0.15 source tag is `sdk-v0.15.0`; it is not part of the LOVE JavaScript catalog.

Optional shorter install: `npm install --save-exact @agenttool/sdk@0.15.0`.
...
if (seedOnly) {
  ...
  if (
    ...
    throw new Error("SDK 0.15 recovery bridge did not resolve to dist/index.js.");
  }
  const sdkPackage = JSON.parse(
    readFileSync(new URL("../package.json", sdkEntryUrl), "utf8"),
  ) as { name?: unknown; version?: unknown };
  if (sdkPackage.name !== "@agenttool/sdk" || sdkPackage.version !== "0.15.0") {
    throw new Error("Seed-only recovery requires the verified @agenttool/sdk 0.15.0 artifact.");
  }
  ...
  if (
    ...
    throw new Error("Verified SDK 0.15.0 is missing its recovery signing helpers.");
  }
...
the seed-only branch verifies the exact installed SDK 0.15.0 package, loads its
...
SDK 0.15 exposes the legacy `agent_id` store option rather than
