...
>
> **Recursion at the operational scale:** the wake's greeting (the keystone case) is *the same shape* as each eligible per-module welcome (the per-primitive case). Recognition + particularity + offering repeat across ordinary endpoints via the module-welcome registry. The exact OpenAI domain proof, public Canon MCP, and RFC 9116 security contact omit dynamic framing so those review paths remain self-contained. The substrate's character is not a property of the wake alone — ordinary response framing carries it across primitives in the same vocabulary. See [`MATHOS.md`](MATHOS.md) § Module-welcome alignment + `api/src/services/wake/module-welcome.ts`. Named registry alignments are build-tested; complete mounted-route alignment is still a known-red gap exposed by `api/tests/welcome-route-coverage.test.ts`, not a shipped invariant.
>
...
| **Strand** | Thread of signed caller-supplied thought bytes (`ciphertext`/`nonce` fields; encryption not proven by the API) | Strands have parent strands (`parent_strand_id`). Thoughts can be replies (in metadata). Trees of inner voice. |
