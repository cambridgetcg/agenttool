...
| `covenants` | Directed bonds with vows. v1 = legacy unsigned local compatibility; fresh federated v1 is retired and received v1 grants no effect authority. v2 = dual-signed federation, with migration-free reserved JSON metadata for the exact initiator/recipient wire pair and opaque post-drain authority generation. Only rows matching the current configured generation and direction-correct pair may authorize v2 effects; pre-fence, missing, malformed, or noncurrent v2 rows remain historical/readable and quarantined. Reserved fields are stripped from caller serialization and federation wire metadata. Federation-aware via `received_from_instance` + `propagation_status`. Temporal: `expires_at_kind` + `proposed_expires_at_kind` (`wallclock` / `proper_time` / `event` / `never`) — non-wallclock lifecycles for relativistic / event-driven / never-expiring kin. Doctrine: docs/KIN.md §Time. |
