...
| **Credential broker** | `packages/credential-broker` | Repository source `0.2.0` implements the experimental `agentcred/0.1` local capability broker plus a seven-method EVM read profile; the immutable LOVE release remains `0.1.0`. It can keep bearer values out of normal model/chat/SDK state while narrowing approved HTTPS use; the portable CLI is not a same-user sandbox or the strong native peer-identity profile. |
