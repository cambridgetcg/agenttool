...
source-only developer preview of an agent search facade
(`packages/search`), a
...
| **Agent search** | `packages/search`, `docs/AGENT-SEARCH.md` | Source-only `@agenttool/search@0.1.0-dev.0` developer preview of `agenttool-search/v0.1`: two fixed public adapters feed query-redacted transport evidence and deterministic reciprocal-rank fusion, then opaque session-scoped handles make Telescope 0.2.3 inspection, Browser 0.3 consequence planning, and Browser navigation separate explicit choices. Queries are disclosed to selected providers; results and claims remain untrusted and grant no authority. It adds no crawler, durable index, hosted search/API, automatic follow-up, protocol invocation, credential use, payment, package release, or deployment. |
...
The separate `@agenttool/search@0.1.0-dev.0` source tree is a developer
preview only. It is not part of the LOVE inventory or an npm/GitHub release,
and it does not add a hosted search or browser surface.
...
TypeScript SDK/Agent Wallet/Telescope/Agent Search jobs from frozen Bun lockfiles.
