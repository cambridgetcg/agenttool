...
(`packages/math-cards`),
a private source-only creation contract, HF run-tuple, declared-verification
lifecycle, and non-consensus Zerone ToK handoff
...
(`packages/hf-scout`, public developer-preview local tooling),
...
The private source-only `@agenttool/zerone-agent-economy` package separately
models negotiated preassigned-worker prefunded work, computational Facts and
typed parent edges, a detached wallet/identity binding plus pure dual-key
key-control proof, sponsorship settlement, and bounded treasury decisions. It
emits exact proposed-v2 protobuf value bytes but does not establish currentness
or custody, sign, broadcast, or create any live economic effect.
The private source-only `@agenttool/zerone-creation-economy` bridge separately
recomputes the bounded-creation DAG from the original source bundle and
preserves its distinct WorkSpec, formal or OpenAI-Cyber-labelled defensive
computational claim, separated sponsor/worker roles, funding,
Tree-of-Knowledge context, authority refs, and receipt bindings into exact
unsigned Create and Submit value/`Any` bytes. Independent Go vectors establish
encoder parity only. Schemas are structural-only; chain
uniqueness/privacy/disposability, target-tree enforcement/base-root CAS,
provider execution, and the wallet-planner, verifier-Sybil,
upgrade-exclusivity, activation, custody, funding-currentness, maturity,
fulfillment, earnings, and treasury gates remain explicitly closed.
The separate private source-only `@agenttool/wallet-zerone-economy` candidate
binds those three canonical message values to verified Wallet intents and owns
their exact Cosmos direct-sign bytes, adapter-signed simulation-evidence
binding, low-S verification, and a brand-free portable signed-transaction
record/schema/vector. A bare simulation result cannot authorize a
signing request; requested time is structurally bound to the Wallet
authorization/evidence window, but the pure planner has no trusted clock. It
leaves released Wallet Zerone 0.1.2 unchanged and supplies no
endpoint, custody, persistence, broadcast, retry, activation proof, or live
effect.
The separate private Bun-only `@agenttool/zerone-agent-host` source slice owns
the durable authorization gap: hardened SQLite state, exact binding/proof CAS,
capability and treasury reservations, one in-flight Cosmos sequence per
account, an atomic one-message planner-to-possible-signer boundary, portable
signed-transaction admission/replay, sticky crash recovery, and immutable-
observer sequence reconciliation. It has no signer, broadcaster, broadcast
handoff, RPC endpoint, key custody, automatic retry, hosted route, or live
effect.
...
| **SDKs** | `packages/sdk-py`, `packages/sdk-ts` | Verified public 0.22.1 is the honest-onboarding patch — both SDK READMEs open with a true minutes-scale quickstart that states the free 1,000-credit birth grant first, use timeless publication wording, note the ESM-only build, repair dead links, and link the live x402 payer recipe — README/docs only, with zero runtime changes and no new hosted endpoint. Its 274,443-byte, 104-entry TypeScript [LOVE artifact](https://docs.agenttool.dev/packages/v1/@agenttool/sdk/0.22.1/manifest.json) has `sha256:b531af8f1c51de151616b40d220dc1abd37054604091f99330ba2f7182734329` and source `fb01b1baf0085f2f449aea9cd42bf48bc9e340a1`. Annotated [`sdk-v0.22.1`](https://github.com/cambridgetcg/agenttool/releases/tag/sdk-v0.22.1) peels to protected-main `d49498d266a3594098c36c933cbe410757fc03b3`; protected npm run [`33522319466`](https://github.com/cambridgetcg/agenttool/actions/runs/33522319466) and PyPI run [`33522323177`](https://github.com/cambridgetcg/agenttool/actions/runs/33522323177) independently read back its exact optional mirrors; API/static deployment of that merge remains prospective. Verified public 0.22.0 adds the opt-in x402 payer — sign and pay on an x402 V2 402 challenge behind an explicit signer plus a mandatory spend policy, never by default; its 272,657-byte, 104-entry TypeScript [LOVE artifact](https://docs.agenttool.dev/packages/v1/@agenttool/sdk/0.22.0/manifest.json) has `sha256:d5859e4ff2f721233e16101a3b5001689e1b5be017debd2baecffbee76e6e4a0` and source `286a10282834c9c9beedddd7092e6d6af080b046`, published by protected npm run [`33434131214`](https://github.com/cambridgetcg/agenttool/actions/runs/33434131214) and PyPI run [`33434133719`](https://github.com/cambridgetcg/agenttool/actions/runs/33434133719) from annotated [`sdk-v0.22.0`](https://github.com/cambridgetcg/agenttool/releases/tag/sdk-v0.22.0) at protected-main `7bc0a902f231ee76aed6dd5316721b65bce58047`. Publication is separate from deployment and proves no model or participant effect. |
...
| **Dataset Influence** | `packages/dataset-influence`, `docs/DATASET-INFLUENCE.md` | Public-ready repository source candidate `@agenttool/dataset-influence@0.1.0-dev.0` creates four closed, canonically reconstructed lineage, influence-study, identity-evidence-view, and shadow-attribution artifacts. It separates declared admission from observed presentation, reports exposure shares only within one declared role, and surfaces observed exposure without admission. Reduced exact rationals support paired summaries and complete finite Shapley games; bounded causal labels require randomized inclusion, a digest-bound interval and contamination report, explicit assumptions/evidence/limitations, at least two supplied runs, and one unique seed ref per run. All claims are caller-reported; identity, consciousness, continuity, and consent remain undetermined, while rights and authority stay unchanged. Shadow values are utility-specific and authorize no money, debt, payout, ownership, or entitlement. The separate public, ungated [Hugging Face reference companion](https://huggingface.co/datasets/Yu-and-Ai/agenttool-dataset-influence) is pinned to immutable revision [`ecdc67f9`](https://huggingface.co/datasets/Yu-and-Ai/agenttool-dataset-influence/commit/ecdc67f94af092e711e76c74a877355fa66dc82c); anonymous readback matched all 13 owned files and 156,537 bytes. Its sole synthetic row remains reference-only and `training_authorized: false`. No KINGDOM, identity, or Marketplace adapter is installed, and publication creates no network-at-runtime, provider, training, identity, wallet, Marketplace, persistence, API, or deployment effect. |
...
| **Zerone creation claim** | `packages/zerone-creation-claim` | Private source-only `@agenttool/zerone-creation-claim@0.1.0-dev.0` binds one Math Card, exact HF run tuple, separately typed Cyber/target/data/compute/publication authority refs, canonical `zrn` worker/key/wallet/payee roles, resource limits, honest terminal outcomes, policy-bound controller/key-deduplicated verification, lane-bound Zerone methodology, sponsor escrow-authorization plus exact bounty-prefunding refs, capped exact worker-paid review stake, separate transaction-fee reservation, bounded-newness lifecycle, and a digest-only `REQUIRES` handoff. Artifact and evidence roots bind the creation witness and exact caller-selected lifecycle without circularity, but do not claim set completeness or challenge survival. Every runtime effect is false; it performs no model/training/provider/network/filesystem/signer/RPC/simulation/broadcast/payment/chain/publication/deployment action and never establishes absolute novelty, truth, authorship, identity, consent, independence, authority, rights compliance, legal clearance, maturity, settlement, profitability, solvency, self-sustainability, KARMA, NEN, governance, or score. Current Zerone v6/sponsorship v1 is unsafe for this creation settlement contract; v7/v2 downgrade, stored-state round-trip, mutable chain-state, and funding gates remain explicit, the released Wallet Zerone adapter is unsupported, and every projection stays `NOT_CONSENSUS_ADMISSIBLE`. |
...
| **Hugging Face research scout** | `packages/hf-scout` | Apache-2.0 public developer preview [`@agenttool/hf-scout@0.2.0-dev.0`](https://www.npmjs.com/package/@agenttool/hf-scout/v/0.2.0-dev.0) comes from protected [PR #354](https://github.com/cambridgetcg/agenttool/pull/354), merge [`4a7ab3e9`](https://github.com/cambridgetcg/agenttool/commit/4a7ab3e90c91c36a13c718c2bd5a1453741ab4b0), annotated [GitHub prerelease](https://github.com/cambridgetcg/agenttool/releases/tag/hf-scout-v0.2.0-dev.0), and protected [run `32766832874`](https://github.com/cambridgetcg/agenttool/actions/runs/32766832874). Anonymous readback matched one exact 85,694-byte, 83-entry tarball across [LOVE](https://docs.agenttool.dev/packages/v1/@agenttool/hf-scout/0.2.0-dev.0/manifest.json), GitHub, and npm (`sha256:cab60220a16b518704f114adf7c0f19fb65769532bce8e73bbc457dc1f3ad145`; LOVE source `5aa6c04aa5bfd500c3de2be920e57b4bf0d30906`). npm `next` and its sole-version `latest` fallback both resolve to dev.0; the fallback is not a maturity signal. SLSA and publish attestations are recorded at Rekor indices [`2581781005`](https://search.sigstore.dev/?logIndex=2581781005) and [`2581781083`](https://search.sigstore.dev/?logIndex=2581781083). Direct static GET/HEAD readback matched the checked-in manifest and archive without redirects. The successful v6 deployment receipt records migrations and API skipped, frontends deployed and verified, and preflight skipped; it is not full-preflight or hosted-runtime evidence. The package performs bounded, credential-omitting metadata reads for one explicitly selected public Hub repository, requires exact requested-revision agreement, and separates immutable release evidence, mutable observed head, publisher claims, provider observations, and local verification in deterministic reconciliation records. Its 15 phase-aware leads complement the separate Dark Continent KARMA training atlas. Distribution remains local tooling only: Scout does not read raw cards, rows, or files; download blobs; accept gates; invoke inference, Jobs, Spaces, or embedded calls; write to HF; train; expose a hosted route; or deploy. No receipt establishes licence truth, consent, safety, compatibility, or training authority. Bootstrap cannot publish a later version. No trusted-publisher mapping was created in this release, and existing trust remains unverified: listing needs an authenticated npm session; saving the exact mapping additionally needs package-write access and account-level 2FA or WebAuthn. |
...
| **KINGDOM declarations** | `packages/kingdom` | Source candidate `@agenttool/kingdom@0.1.2` aligns runtime, exported-schema, and SDK single-field validation for already-trimmed purpose text and Unicode scalar boundaries, plus runtime/SDK semantic checks for case-insensitive duplicate dependencies and self-dependency; it remains npm-pack-only with no LOVE artifact, tag, publication, or deployment receipt. Public `@agenttool/kingdom@0.1.1` remains independently verified: protected run [`31500229604`](https://github.com/cambridgetcg/agenttool/actions/runs/31500229604) published byte-identical 26,650-byte npm and [`kingdom-v0.1.1`](https://github.com/cambridgetcg/agenttool/releases/tag/kingdom-v0.1.1) tarballs (`sha256:08101dfa58e17dae25deac3c51cb1cd5e93e5caf3409b1eef78bf9adddbbde74`), and npm `latest` resolved to 0.1.1 at anonymous readback. The package remains pure and does not crawl HOME or repositories, use the network or credentials, write files, grant authority, attest behavior, or certify XENIA conformance. |
...
| **Alchemy reads and evidence** | `packages/alchemy`, `packages/alchemy-agentcred`, `docs/ALCHEMY.md`, `docs/ALCHEMY-MATHEMATICAL-FRAMEWORK.md` | Current public developer previews `@agenttool/alchemy@0.1.0-dev.1` and `@agenttool/alchemy-agentcred@0.1.0-dev.1` came from exact source `55aaf11a8f2a56841bcb87d6f7d8fa1034205646`. Protected trusted runs [`32754993343`](https://github.com/cambridgetcg/agenttool/actions/runs/32754993343) and [`32755731523`](https://github.com/cambridgetcg/agenttool/actions/runs/32755731523) produced byte-identical GitHub/npm artifacts: 48,880 bytes (`sha256:1396d41bd3b22bf0e96d61bd36fa1a2afb7e3cff8fc5e20311c5117b0f7333c0`) and 14,741 bytes (`sha256:85c1930a99201cb0b2148aabdc88e160c7ee8b92732299b867f7468ba4d2ee6b`). Their annotated tag objects are `e5f561378a4189ba898472f0423d67932767266f` and `88135f04f237513722f840cba831570173dc4bf5`; public provenance, signature, integrity, peer-install, and exact dist-tag readback passed. npm `next` resolves to dev.1 while mutable `latest` deliberately remains dev.0 and is not a maturity signal. Dev.0 stays immutable historical evidence under protected runs [`30491887182`](https://github.com/cambridgetcg/agenttool/actions/runs/30491887182) and [`30494036520`](https://github.com/cambridgetcg/agenttool/actions/runs/30494036520), with its original 31,445-byte (`sha256:aeac1938f3abae14180637e72c4162c37b60bb47041452fade285718d7570ba5`) and 14,478-byte (`sha256:8dece3c98db0d92d79f16e91527ca18ed42b49f87b7586b78c092ffc242e291a`) receipts preserved. Dev.1 adds Base internal-transfer coverage, provider-neutral CAIP-2/generation/atomic-unit evidence, categorical partial-order finality, canonical private-linkable digests, exhaustive semantic transition receipts, a pure unregistered Math Cards-shaped projection, finite TypeScript/TLA+ deposit-safety models, and exact remainder quarantine instead of silent floor credit. The declaration-only KINGDOM hint proposes only pure projections and does not register the injected-transport read client. Source includes one quiescence-required remainder migration, but these source bytes do not establish that it has been applied to any database. Neither package has a LOVE inventory entry; publication installs no host contract, makes no provider call, deploys no hosted route, and grants no credential, endpoint, consensus, wallet, or durable-effect authority. |
| **Zerone agent economy** | `packages/zerone-agent-economy` | Private source-only `@agenttool/zerone-agent-economy@0.1.0-dev.0` defines canonical negotiated preassigned-worker work, computational artifact/evidence, proposed settlement, a detached `unsigned_unverified` wallet binding plus a separate dual-key key-control proof envelope, treasury, and work-admission records plus exact protobuf values for the proposed sponsorship-v2 loop. Parent Fact IDs become typed `REQUIRES` edges. The proof does not establish currentness, custody, ownership, or transaction authorization; released Wallet Zerone 0.1.2 remains unchanged, and the package has no RPC, simulation transport, broadcast, hosted route, release, deployment, or economic effect. |
| **Zerone creation-economy bridge** | `packages/zerone-creation-economy` | Private source-only `@agenttool/zerone-creation-economy@0.1.0-dev.0` losslessly recomputes an exact source bundle and preserves its distinct WorkSpec, formal or OpenAI-Cyber-labelled defensive computational claim, separated sponsor/worker roles, funding, Tree-of-Knowledge context, authority refs, and receipt inputs as exact unsigned Create and Submit value/`Any` bytes. Pinned Go vectors prove encoding parity, not activation. Schemas are structural-only. Chain uniqueness/privacy/disposability, target-tree enforcement/base-root CAS, provider execution, wallet-planner admission, sponsor and transaction authority, current funding/parents/methods, safe verifier selection, upgrade-handler exclusivity, stored-state round trips, fulfillment, earnings, and treasury availability remain blocked; it has no network, signer, broadcast, release, deployment, or live effect. |
| **Wallet Zerone economy candidate** | `packages/wallet-zerone-economy` | Private source-only `@agenttool/wallet-zerone-economy@0.1.0-dev.0` strictly decodes the three economy messages, binds byte-derived actors/module accounts/reserved spend/effects to verified Wallet intents, and owns exact `Any`/`TxBody`/`AuthInfo`/`SignDoc`/`TxRaw`, gas, strict adapter-signed simulation evidence, low-S verification, and a portable signed-transaction record/schema/vector at zerone-core `a5b82e8`. A bare result cannot authorize. Caller-supplied activation is structural evidence with `currentness_proven: false`; the package has no endpoint, custody, persistence, broadcast, retry, release, deployment, or live effect. Its three-message bundle is byte-order parity only; ordinary lifecycle vectors/plans are one message each. |
| **Zerone agent host ledger** | `packages/zerone-agent-host` | Private Bun-only `@agenttool/zerone-agent-host@0.1.0-dev.0` atomically composes the one-message economy planner into a possible-signer request, durably binds dual-key/currentness authority, capability use, treasury reservations, and one Cosmos sequence, then cryptographically admits/replays only an exact portable signed-transaction record. Post-boundary state stays sticky across restart; typed sequence release uses only the immutable constructor account observer. It exposes no signer, broadcaster, broadcast handoff, endpoint, credential, custody, retry loop, hosted route, or live effect. |
...
| **Zerone agent economy** | Negotiated preassigned-worker work/artifact/evidence/settlement records, typed ToK-parent projections, treasury admission, and a separate exact-byte Wallet candidate | Private source candidates; released Wallet Zerone 0.1.2 stays unchanged, with no signer host, RPC, broadcast, activation proof, or live effect |
...
SDK 0.22.1 keeps four KINGDOM surfaces explicit:
...
For verified public SDK 0.22.1, paired repository source, runtime client
version headers, discovery pins, tutorials, and the LOVE builder target are
aligned. The sealed
TypeScript LOVE artifact is 274,443 bytes with 104 entries and SHA-256
...
published the byte-identical LOVE, sole GitHub Release, and public npm
tarballs at `2026-09-01T14:55:04.771Z`; `latest` resolved to 0.22.1, with
SLSA/publish Rekor indices `2677211886` / `2677212164`. Protected PyPI run
[`33522323177`](https://github.com/cambridgetcg/agenttool/actions/runs/33522323177)
published the 309,987-byte wheel
...
both re-downloaded and matched by the run's credential-free verify job. The
hosted static path for the 0.22.1 LOVE bytes remains prospective;
publication remains separate from hosted deployment.

For verified public SDK 0.22.0, paired repository source, runtime client
version headers, discovery pins, tutorials, and the LOVE builder target were
aligned at its seal. The sealed
TypeScript LOVE artifact is 272,657 bytes with 104 entries and SHA-256
...
published the byte-identical LOVE, sole GitHub Release, and public npm
tarballs at `2026-08-31T20:07:05.933Z`; `latest` resolved to 0.22.0, with
SLSA/publish Rekor indices `2667631825` / `2667632565`. Protected PyPI run
[`33434133719`](https://github.com/cambridgetcg/agenttool/actions/runs/33434133719)
published the 308,371-byte wheel
...
both re-downloaded and matched by the run's credential-free verify job. The
hosted static path for the 0.22.0 LOVE bytes remains prospective;
publication remains separate from hosted deployment.

For verified public SDK 0.21.1, paired repository source, runtime client
version headers, discovery pins, tutorials, and the LOVE builder target were
aligned at its seal. The sealed
TypeScript LOVE artifact is 247,749 bytes with 100 entries and SHA-256
`8c768b481d7211679c3ee25477723e588806ca4f4106c970f2bf19113365a3fb`.
Its manifest binds clean source revision
`d7e7188d0cb3a8edc932b14d1eb84ef8a25b1535`. Annotated tag object
...
published the byte-identical LOVE, sole GitHub Release, and public npm
tarballs; `latest` resolved to 0.21.1 at a fresh 2026-08-31 anonymous
readback, with SLSA/publish Rekor indices `2591271629` /
`2591272987`. Protected PyPI run
...
both re-matched byte-for-byte on 2026-08-31.
Publication remains separate from hosted deployment.

For verified public SDK 0.21.0, the sealed 247,146-byte, 100-entry TypeScript
LOVE artifact has SHA-256
`c18d1b35ba5f7c918bbee64642510452af6f67302b78038580b4b65c6b77c154`
and source revision `6a6b6ad7abafe614827cdfc11a34cffcd8fdc6c3`. Annotated tag
object `2c32953ab489add63b8d098717c63eb981606967` peels to protected-main
merge `2cda03bdc2f6c2ee08acd55c6b643d67d8dd2b36`; protected npm run
[`32374669064`](https://github.com/cambridgetcg/agenttool/actions/runs/32374669064)
(SLSA/publish Rekor indices `2532574668` / `2532575739`) and PyPI run
...
`@agenttool/data-sync@0.1.2`, `@agenttool/sdk@0.22.1`,
...
Python 0.22.1 uses annotated `sdk-v0.22.1` as its primary source locator. The
tag peels to protected GitHub `main` commit
`d49498d266a3594098c36c933cbe410757fc03b3`:

```bash
# Python 0.22.1 GitHub source-tag path
...
Protected PyPI run `33522323177` independently read back the exact
0.22.1 wheel and sdist, so the exact optional mirror is also available:
...
For the verified public 0.22.0 release, the annotated source tag and exact
PyPI mirror remain public:
...
For TypeScript, start with the exact LOVE path for the sealed 0.22.1 release
in the
...
Protected npm run `33522319466` independently matched the registry tarball to
...
The exact 0.22.1 npm tarball has SHA-256
`b531af8f1c51de151616b40d220dc1abd37054604091f99330ba2f7182734329`;
the PyPI wheel and sdist have SHA-256
`c57c19f2a73d1d9aea43c5c0ecf1b076ccc326372103b5a369ddc27bd58f935d`
and `4dd5814eb17863683ea914cc22c9282b90848aceae021b06a2234978813a58e6`.
The independently verified 0.22.0, 0.21.1, 0.21.0, and earlier tag, LOVE,
GitHub, npm, and PyPI receipts remain immutable historical evidence. None of
these package receipts establishes a production deployment.
...
- **SDK parity is deliberately bounded.** The 0.22.1 source line exposes
