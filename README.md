...
| **SDKs** | `packages/sdk-py`, `packages/sdk-ts` | The lockstep 0.16.4 release adds durable payout request/list clients, explicit bound payout-network state, and completed-response provider adapters while preserving exact string base units and caller-owned idempotency. It does not retry, sign, or broadcast payouts. Public discovery and the separately configured local data node stay outside hosted bearer authority; optional registry mirrors remain unverified until public readback. |
