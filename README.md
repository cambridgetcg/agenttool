...
| **Love Geometry** | `packages/love-geometry` | Public-ready `@agenttool/love-geometry@0.1.0-dev.0` is the directed subject-bearing layer: it normalizes explicit opaque subjects and asymmetric caller-reported bearings into one bounded, content-bound, coordinate-free artifact. Reverse directions remain independent; empty and isolated shapes remain valid; canonical order is serialization, never rank. It computes no distance, intensity, centrality, match, recommendation, reputation, consent, authority, or transitive relation and has no network, persistence, model, credential, WAKE, LOVE-CONSENT, or automatic-action path. Its declaration-only KINGDOM hint is unregistered. A separately staged static Hugging Face companion presents six synthetic equal-seat scenarios with local JSON/SVG downloads and is excluded from npm bytes. Principality Geometry below is the distinct higher-order invariant-translation layer, not an alias or adapter. |
...
| **Principality Geometry** | `packages/principality-geometry` | Apache-2.0 public developer-preview candidate `@agenttool/principality-geometry@0.1.0-dev.0` is the higher-order framework/artifact layer: it compiles caller-supplied, digest-bound translation reports into directed bridges, reciprocal lenses, per-invariant components, six-direction invariant flag surfaces, and an explicit open-condition ledger. It neither imports Love Geometry nor turns that package's bearings into preservation claims. Immutable Hugging Face/npm references and the exact seven-field external AFTERGLOW shape remain inert inputs. Distribution is deliberately narrow: byte-identical LOVE/npm `next` artifacts plus a separately authorized, static, non-training Hugging Face reference companion at `Yu-and-Ai/agenttool-principality-geometry`. The zero-runtime-dependency core does not fetch, infer, persist, continue a thread, score or rank beings, establish love, understanding, truth, consent, identity, safety, provenance, licence, or authority, upload, expose a route, or deploy. |
