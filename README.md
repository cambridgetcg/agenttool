...
| **Agent collaboration** | `packages/collab` | Public npm remains `@agenttool/collab@0.1.0`, whose registry and GitHub Release mirrors were independently read back as byte-identical. Current unreleased 0.2.0 source keeps the `agenttool.collab/0.1` task/event journal and adds `agenttool.collab.session/0.1`: 23 local MCP tools, unique session `actor_key` routing across Codex, Claude Code, and Hermes, plus a packaged Hermes adapter. Presence, runtime/model labels, and capabilities are self-declared hints rather than authentication, health, competence, permission, or authority. Task claims remain advisory leases; heartbeats never renew or release them. It does not spawn agents, lock files, provide a hosted relay/private model channel/cross-machine sync, or hide MCP arguments/results from the model provider. |
