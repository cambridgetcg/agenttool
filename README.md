...
| **Alchemy reads** | `packages/alchemy`, `docs/ALCHEMY.md` | Developer-preview `@agenttool/alchemy@0.1.0-dev.0` source provides eight bounded EVM read/reconciliation operations through an injected host-owned transport. It has no generic RPC, URL/key/header input, signer, broadcaster, retry, webhook/admin path, hosted route, MCP surface, registry release, or deployment. Live RPC, provider safe/finalized tags, numbered blocks, and indexed transfers retain distinct provenance caveats. |
