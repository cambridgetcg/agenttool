...
| **SDKs** | `packages/sdk-py`, `packages/sdk-ts` | Paired 0.18.0 source and the checked-in TypeScript [LOVE release](https://docs.agenttool.dev/packages/v1/@agenttool/sdk/0.18.0/manifest.json) add `attestationMarketplace` / `attestation_marketplace`, `memoryWitness` / `memory_witness`, and `syneidesis`, alongside shared URL/error boundaries and selected cross-language fixtures. Settlement remains evidence rather than truth, Syneidesis project-bearer records are not signatures, and model-authored Anthropic chronicle writes require an explicit review hook. The bounded KINGDOM reads remain separate. 0.18.0 registry availability is not inferred from source or LOVE; the exact 0.17.0 npm and PyPI receipts remain the last independently verified public mirrors. |
...
| **Agent collaboration** | `packages/collab` | Source candidate `@agenttool/collab@0.3.2` adds the 32nd local MCP tool, read-only `collab_anchor_status`, for bounded comparison with an optional local sidecar ledger. It never contacts Zerone and a result does not prove remote chain state. Public `0.3.1` remains the last independently verified npm/GitHub release: protected run [`30389483811`](https://github.com/cambridgetcg/agenttool/actions/runs/30389483811), 296,260 byte-identical bytes, `sha256:dd0b0a0897a6d414e013e7f80b29ed9b200f94b3bcfe9d79598bc50b619db6ee`. Claims remain advisory; Collab does not spawn agents, lock files, host a relay, create a private model channel, or add a Fly/API surface. |
