...
record; npm and GitHub mirrors remain independently unverified. Immutable
0.2.2 remains available with its historical permissive-exit flaw, while the
current producer remains compatible with 0.2.1. Telescope remains a local
...
| **SDKs** | `packages/sdk-py`, `packages/sdk-ts` | The lockstep 0.16.4 release adds durable payout request/list clients, explicit bound payout-network state, and completed-response provider adapters while preserving exact string base units and caller-owned idempotency. Hosted fresh payout admission is resting because historical ledger labels did not conserve cashable backing; existing rows remain listable/replayable. The SDK does not retry, sign, or broadcast payouts. Public discovery and the separately configured local data node stay outside hosted bearer authority; optional registry mirrors remain unverified until public readback. |
...
| **Credential broker** | `packages/credential-broker` | Repository source and the exact immutable LOVE artifact are `0.2.0`, implementing the experimental `agentcred/0.1` local capability broker plus a seven-method EVM read profile; npm publication remains a separate protected workflow. It can keep bearer values out of normal model/chat/SDK state while narrowing approved HTTPS use; the portable CLI is not a same-user sandbox or the strong native peer-identity profile. |
...
| **Alchemy reads** | `packages/alchemy`, `docs/ALCHEMY.md` | Developer-preview `@agenttool/alchemy@0.1.0-dev.0` source permits eight bounded provider methods plus opaque same-client transfer continuation through an injected host-owned transport. It is an observation client, not a durable reconciliation engine, and has no generic RPC, URL/key/header input, signer, broadcaster, retry, webhook/admin path, hosted route, MCP surface, registry release, or deployment. Live RPC, provider safe/finalized tags, numbered blocks, and indexed transfers retain distinct provenance caveats. |
...
For SDK 0.16.4, repository source manifests and runtime client version headers
...
`@agenttool/data-sync@0.1.1`, `@agenttool/sdk@0.16.4`,
...
```bash
# Python 0.16.4 GitHub source tag (release path, not a PyPI publication claim)
...
python -m pip install "agenttool-sdk==0.16.4"
```
...
```bash
npm view @agenttool/sdk@0.16.4 version --registry=https://registry.npmjs.org
...
- **SDK parity is deliberately bounded.** The 0.16.4 source line exposes `at.data`
