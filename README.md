...
| **Credential broker** | `packages/credential-broker` | Repository source, the exact immutable LOVE artifact, and the [`credential-broker-v0.2.0` GitHub Release](https://github.com/cambridgetcg/agenttool/releases/tag/credential-broker-v0.2.0) are `0.2.0`; the release asset is `sha256:bfe8fec121ec9d2fe46946b8f0fa89691193dc0a10de1a282a9ad361abe352ba`. The public npm package remains at `0.1.0` because the protected 0.2.0 publication was not authorized for that registry namespace. The package implements the experimental `agentcred/0.1` local capability broker plus a seven-method EVM read profile. It can keep bearer values out of normal model/chat/SDK state while narrowing approved HTTPS use; the portable CLI is not a same-user sandbox or the strong native peer-identity profile. |
...
`@agenttool/credential-broker@0.2.0`, `@agenttool/wallet@0.1.3`,
