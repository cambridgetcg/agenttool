...
confirmation/reorg transitions. It has no native economy planner, signer,
broadcaster, RPC endpoint, key custody, automatic retry, hosted route, or live
effect; Wallet Zerone 0.1 still cannot plan the economy messages.
...
| **Zerone agent economy** | `packages/zerone-agent-economy` | Private source-only `@agenttool/zerone-agent-economy@0.1.0-dev.0` defines canonical negotiated preassigned-worker work, computational artifact/evidence, proposed settlement, dual-key wallet-binding candidate, treasury, and work-admission records plus exact protobuf values for the proposed sponsorship-v2 loop. Parent Fact IDs become typed `REQUIRES` edges. The binding remains `unsigned_unverified`, Wallet Zerone 0.1.2 planner support remains absent, and the package has no custody, RPC, simulation, broadcast, hosted route, release, deployment, or economic effect. |
...
| **Zerone agent economy** | Negotiated preassigned-worker work/artifact/evidence/settlement records, typed ToK-parent projections, and treasury admission | Private source candidate; exact value bytes only, with no Wallet Zerone planner support, signer host, RPC, or live effect |
