...
| **Polymorph Landscape** | `packages/polymorph-landscape`, `docs/POLYMORPH-LANDSCAPE.md` | Source-only `@agenttool/polymorph-landscape@0.1.0-dev.0` provides three closed `/0.1` landscape, reachability-shift, and lesson formats plus authored `en`, `yue-Hant`, `zh-Hant`, and `zh-Hans` lessons. Ritonavir “disappearance” is encoded as named-condition nonreproduction with source-scoped form labels, unresolved causation, and no erasure or inevitability claim. The checked-in static HF companion performs no upload or training; the protected npm `next` path is a candidate, not a publication. WAKE explicitly copies declared wall URNs through software, which establishes no identity, memory/continuity, consent, permission, or inherited authority. |
