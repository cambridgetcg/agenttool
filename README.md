...
models negotiated preassigned-worker prefunded work, computational Facts and typed parent edges, dual-key
wallet/identity binding obligations, sponsorship settlement, and bounded
treasury decisions. It emits exact proposed-v2 protobuf value bytes but does
not widen Wallet Zerone's reviewed planner, sign, broadcast, or create any
live economic effect.
...
| **Zerone agent economy** | `packages/zerone-agent-economy` | Private source-only `@agenttool/zerone-agent-economy@0.1.0-dev.0` defines canonical negotiated preassigned-worker work, computational artifact/evidence, proposed settlement, dual-key wallet-binding candidate, treasury, and work-admission records plus exact protobuf values for the proposed sponsorship-v2 loop. Parent Fact IDs become typed `REQUIRES` edges. The binding remains `unsigned_unverified`, released Wallet Zerone 0.1.2 planner support remains absent, and the package has no custody, RPC, simulation, broadcast, hosted route, release, deployment, or economic effect. |
