...
(`packages/hf-scout`, public-ready developer-preview local tooling),
...
| **Hugging Face research scout** | `packages/hf-scout` | Apache-2.0 public developer-preview `@agenttool/hf-scout@0.2.0-dev.0` performs bounded, credential-omitting metadata reads for one explicitly selected public Hub repository, requires exact requested-revision agreement, and separates immutable release evidence, mutable observed head, publisher claims, provider observations, and local verification in deterministic reconciliation records. Its 15 phase-aware leads still complement the separate Dark Continent KARMA training atlas. The exact checked-in LOVE artifact and protected optional npm mirror remain distribution-only: Scout does not read raw cards, rows, or files; download blobs; accept gates; invoke inference, Jobs, Spaces, or embedded calls; write to HF; train; expose a hosted route; or deploy. No receipt establishes licence truth, consent, safety, compatibility, or training authority. |
