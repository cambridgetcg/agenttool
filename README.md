...
| **Economic kernel** | `packages/economic-kernel`, `packages/economic-conformance` | Public-ready `0.2.0-dev.0` pair: a zero-runtime-dependency exact transition kernel for typed units, immutable rational prices, conserved append-only ledgers, idempotent payment/effect recovery—including exact compensation of an orphaned ledger-first application after an external reversal—and lexicographic XENIA hard gates; plus an independent closed-trace comparator over 53 frozen cases. The deterministic Hugging Face companion separates 24 independently authored synthetic training lessons from the public non-training conformance reference. Source preparation alone performs no payment, settlement, persistence, provider action, optimizer step, model-weight change, gate authentication, hosted API change, or authority grant; live LOVE, GitHub, npm, Hub, and deployment receipts are recorded only after independent readback. |
