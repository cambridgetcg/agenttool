...
`@agenttool/browser@0.5.1` is the current exact LOVE release, with npm and
annotated GitHub Release mirrors carrying the same protected artifact. The
patch adds Codex plugin packaging and an isolated packed MCP bundle without
changing the exact 0.5.0 runtime, tools, protocols, launch authority, or
installed-browser requirement. The static docs/catalog deployment and its
...
| **Agent browser** | `packages/browser`, `docs/AGENT-BROWSER.md` | Current `@agenttool/browser@0.5.1` is one exact LOVE release with npm and annotated GitHub Release mirrors over the same local TypeScript, JSONL, and stdio MCP core. Version 0.5.1 adds a package-root Codex plugin and self-contained Node-targeted MCP bundle that loads from the packed plugin cache without parent `node_modules`; its manifest adds no authority flag, so public, headless, ephemeral defaults and the operator-installed browser boundary remain unchanged. Seven browser operations plus `browser_capabilities` and zero-effect `browser_plan` form a nine-tool agent surface. Exact 0.5.0 carried forward the never-distributed 0.4 standing-policy diagnostic and JSONL rename hint, then added redacted receipts for admitted action attempts, optional retained-observation bases for non-ref actions, bounded receipt context in observations, a backend-neutral operation inventory, and current MCP negotiation with an explicit legacy path. Receipts distinguish rejection before runtime, local browser completion, and uncertain post-dispatch failure; they are retry guidance, not proof of remote effect. Retained snapshots are session-local evidence, not a distributed lease or DOM-equality proof. Launch-time `public`, `local`, and `sovereign` profiles remain explicit; sovereign does not bypass authentication, site, network, or operating-system boundaries. Page data remains untrusted; file upload, automatic download, arbitrary page evaluation, credential injection, and shell access remain unsupported. Public/local DNS preflight does not pin the later browser connection, and Playwright-managed redirects are not independently revalidated, so this is not strong SSRF isolation. Exact 0.5.0, 0.3.0, 0.2.0, and 0.1.0 release bytes remain immutable historical artifacts. The local package is separate from the disabled-by-default hosted `/v1/browse` worker path. |
...
The separate `@agenttool/browser@0.5.1` release is a local runtime with an
exact LOVE record and byte-locked npm/GitHub mirrors. Publication does not add
a hosted browser API. Its Codex plugin runs a self-contained packed MCP bundle
over the unchanged exact 0.5.0 runtime and does not widen launch authority.
...
`@agenttool/browser@0.5.1`. Earlier immutable
