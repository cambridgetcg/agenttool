...
| **SDKs** | `packages/sdk-py`, `packages/sdk-ts` | The lockstep 0.16.4 release adds durable payout request/list clients, explicit bound payout-network state, and completed-response provider adapters while preserving exact string base units and caller-owned idempotency. It does not retry, sign, or broadcast payouts. The TypeScript artifact is public through the [`sdk-v0.16.4` GitHub Release](https://github.com/cambridgetcg/agenttool/releases/tag/sdk-v0.16.4), npm, and LOVE; all three exact-byte reads match `sha256:ab11a7a69c1bb73e0a2aa936131bec4aa2e28db222091311970e012cdb21ea4d`. The Python source shares that immutable tag, but PyPI 0.16.4 remains unavailable because its trusted-publisher mapping did not accept the protected workflow identity. Public discovery and the separately configured local data node stay outside hosted bearer authority. |
