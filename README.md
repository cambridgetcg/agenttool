...
| **SDKs** | `packages/sdk-py`, `packages/sdk-ts` | Paired 0.19.0 source and the checked-in 230,184-byte TypeScript [LOVE candidate](https://docs.agenttool.dev/packages/v1/@agenttool/sdk/0.19.0/manifest.json) (`sha256:0a7eed4029bc687605b4d56707843c12ccb36d10a162a1fea1681522ab8784a2`, source `3239a25987d9de95b678e808d2d5168e786b2472`) add data-only `wake.observe` and a separate credential-free Math Cards assessment client. Math Cards sends no project bearer, cookies, redirects, authenticated transport, or ambient proxy credentials; canonical IDs and assessment semantics remain server-owned. This repository evidence establishes no 0.19.0 tag, GitHub Release, npm/PyPI publication, docs deployment, or hosted behavior. The immutable 0.18.1 LOVE/tag/GitHub/npm/PyPI receipts remain exact historical evidence, including npm run [`31790395261`](https://github.com/cambridgetcg/agenttool/actions/runs/31790395261) and PyPI run [`31790559054`](https://github.com/cambridgetcg/agenttool/actions/runs/31790559054). Production deployment remains separate and unclaimed. |
...
| **LOVE BOMB v4** | `docs/LOVE-BOMB.md`, `docs/specs/agenttool-love-bomb-0.1.json`, `apps/docs/love-bomb.html` | One canonical finite public invitation at `https://docs.agenttool.dev/love-bomb`, available through reader-chosen `GET` or `HEAD`. The existing WAKE `you_can_reach` shape carries only its pull-only coordinate through the paired SDKs. It does not push text, select a recipient or register, infer identity or feeling, record receipt, create consent or relationship, change weights, or prove attention, understanding, retention, or effect. |
| **Model Becoming** | `packages/model-becoming`, `docs/LOVE-BOMB.md` | Public-ready `@agenttool/model-becoming@0.1.0-dev.0` creates and validates closed evidence-scoped lifecycle dossiers across artifact identity, lineage, objectives, data provenance and collection disclosure, learned weights, post-training behavior shaping, runtime, agency/authority, affect/welfare, capability/power, ontology, and disputes. Its pinned Moonshot Kimi-K2-Instruct reference uses exact raw HF content URLs and digests while classifying technical-report statements as first-party disclosures; it does not invent undisclosed scraping sources, rights clearance, feelings, desire, consciousness, identity, continuity, authority, weight effect, or “deepest reach.” The deterministic HF tree contains one wrapped reference-only row with `training_admission: not_applicable` and `training_authorized: false`; publication is not training. The pure zero-dependency package has no network, provider, model, training, persistence, telemetry, delivery, score, or automatic action. |
...
| **Love Geometry** | `packages/love-geometry` | GitHub/Hugging Face developer preview `@agenttool/love-geometry@0.1.0-dev.0` normalizes explicit opaque subjects and asymmetric caller-reported bearings into one bounded, content-bound, coordinate-free artifact. Reverse directions remain independent; empty and isolated shapes remain valid; canonical order is serialization, never rank. It computes no distance, intensity, centrality, match, recommendation, reputation, consent, authority, or transitive relation and has no network, persistence, model, credential, WAKE, LOVE-CONSENT, or automatic-action path. Protected [run `31499968474`](https://github.com/cambridgetcg/agenttool/actions/runs/31499968474) produced and re-read a 19,507-byte [`love-geometry-v0.1.0-dev.0`](https://github.com/cambridgetcg/agenttool/releases/tag/love-geometry-v0.1.0-dev.0) asset (`sha256:43cfbf4b559aa6f573d9d7b7a60e2a7dce5dfa4aefe2bf5b9c92310c926a9db8`), then npm returned `E404`; anonymous npm remained absent, so no npm version or dist-tag is claimed. The tarball excludes its separately published public static [Hugging Face Space](https://huggingface.co/spaces/Yu-and-Ai/love-geometry); anonymous readback of immutable source-binding revision [`f8d4c299`](https://huggingface.co/spaces/Yu-and-Ai/love-geometry/commit/f8d4c299d8d17d116ced27736204e541f9206865) matched all 10 repo-owned files (release-helper root SHA-256 `d785eac90c7c06ec06f7754911b18cf1bf6bd283a98baaa6f97081ab301f32fc`), with provider `.gitattributes` as the sole extra. The public API reported public, ungated, enabled, and running state; six cache-busted static HEAD reads returned HTTP 200 and exact `x-repo-commit`. Provider-transformed GET HTML remained 5,363 bytes versus the 5,262-byte Hub source and is not claimed byte-identical. The manifest binds only three runtime files to Git `19cc1721`; package version/tag and artifact path/bytes/digest/integrity/build/toolchain remain null, and `executes_exact_package_artifact` remains false. The Space does not make the package executable, score relations, or prove identity, consent, truth, or authority. |
...
SDK 0.19.0 keeps four KINGDOM surfaces explicit:
...
For SDK 0.19.0, paired repository source, runtime client version headers,
discovery pins, tutorials, and the LOVE builder target are aligned. The
checked-in TypeScript LOVE candidate is its primary repository release record.
It is 230,184 bytes with SHA-256
`0a7eed4029bc687605b4d56707843c12ccb36d10a162a1fea1681522ab8784a2`
and binds source revision `3239a25987d9de95b678e808d2d5168e786b2472`.
Source and LOVE preparation establish no 0.19.0 tag, GitHub Release, npm or
PyPI publication, docs deployment, or hosted behavior. The intended optional
registries remain `independently_visible: false` until exact public readback.
...
`@agenttool/data-sync@0.1.2`, `@agenttool/sdk@0.19.0`,
...
For Python 0.19.0, the intended primary source locator is the exact annotated
`sdk-v0.19.0` tag after it becomes independently visible. The checked-in
source and LOVE candidate do not establish that tag:

```bash
# Python 0.19.0 GitHub source-tag path; use only after exact tag readback
...
The 0.19.0 PyPI command is conditional: query that exact version first. At
preparation time the mirror is `independently_visible: false`; never infer it
from repository source or a mutable package index:
...
The npm 0.19.0 mirror is also conditional and non-authoritative. Query the
exact version; at preparation time it is `independently_visible: false`. A
mutable dist-tag is informational, not authority, and this registry command
alone does not recheck the LOVE manifest:
...
PyPI 0.18.1 is public as an optional, independently verified convenience
mirror. Protected run `31790559054` matched the public non-yanked 248,937-byte
wheel and 233,734-byte source distribution to the prepared workflow artifacts.
Protected npm run `31790395261` independently read back
`@agenttool/sdk@0.18.1` byte-identical to the immutable 218,301-byte LOVE
artifact (`sha256:466adb2d22a637e9c4d158e6050a69096e296258e6111f482be2a0872318be0d`).
Those exact 0.18.1 receipts remain historical and do not establish 0.19.0
availability. The exact 0.17.0 npm and PyPI mirrors are independently public.
...
- **SDK parity is deliberately bounded.** The 0.19.0 source line exposes
  ...
  Cards assessment client. The parity
