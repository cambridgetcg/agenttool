...
`@agenttool/browser@0.3.0` is published through LOVE/npm with a public docs
...
| **Correspondence projection** | `packages/correspondence-yutabase`, `packages/correspondence-yutabase-projector` | The public metadata-only planner remains available as `@agenttool/correspondence-yutabase@0.1.0-dev.0`; current source prepares the unpublished `0.1.0-dev.1` validation update and still performs no verification or I/O. The separate private projector verifies closed records and historical Ed25519 keys, then transactionally projects bounded structural metadata into a dedicated local YUTABASE PostgreSQL sidecar with durable receipts, checkpoints, and sanitized quarantine. It inherits YUTABASE's named thread-appender capability instead of adding direct core grants, pins the exact core function surface, and enforces a separate exact sidecar ACL. Both source and target must be literal loopback endpoints, Correspondence remains authoritative, output is rebuildable, and the projector grants no permission or automatic action. It has no npm/LOVE release, hosted service, worker, production migration, or deployment surface. |
...
The separate `@agenttool/browser@0.3.0` release is a local runtime with an
...
`@agenttool/browser@0.3.0`. Earlier immutable
