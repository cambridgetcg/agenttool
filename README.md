...
binding, and low-S verification. A bare simulation result cannot authorize a
...
only the durable authorization gap: hardened SQLite state, exact binding/proof
CAS, capability and treasury reservations, one in-flight Cosmos sequence per
account, sticky crash/ambiguity recovery, and injected positive-evidence
confirmation/reorg transitions. The separate exact planner exists, but this
ledger does not yet consume its branded records; it has no signer, broadcaster,
RPC endpoint, key custody, automatic retry, hosted route, or live effect.
...
| **Wallet Zerone economy candidate** | `packages/wallet-zerone-economy` | Private source-only `@agenttool/wallet-zerone-economy@0.1.0-dev.0` strictly decodes the three economy messages, binds byte-derived actors/module accounts/reserved spend/effects to verified Wallet intents, and owns exact `Any`/`TxBody`/`AuthInfo`/`SignDoc`/`TxRaw`, gas, strict adapter-signed simulation evidence, and low-S verification at zerone-core `a5b82e8`. A bare result cannot authorize. Caller-supplied activation is structural evidence with `currentness_proven: false`; the package has no endpoint, custody, persistence, broadcast, retry, release, deployment, or live effect. Its three-message bundle is byte-order parity only; ordinary lifecycle vectors/plans are one message each. |
| **Zerone agent host ledger** | `packages/zerone-agent-host` | Private Bun-only `@agenttool/zerone-agent-host@0.1.0-dev.0` durably CAS-binds an injected proof reference, capability usage, treasury purpose/floor reservations, and one Cosmos sequence per account; post-signer boundaries stay sticky across restart until positive sequence-advance evidence, while inclusion/reorg evidence remains injected. It deliberately exposes no native economy planner, signer, broadcaster, endpoint, credential, custody, retry loop, hosted route, or live effect. |
