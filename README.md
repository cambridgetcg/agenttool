...
| **Agent collaboration** | `packages/collab` | Public `@agenttool/collab@0.3.0` remains npm `latest` with SLSA provenance; its npm and GitHub Release tarballs were byte-identical (`sha256:9c605ebe4cdc87eda1b0eede6bba0a6591a3dd62badd364463b01521401def7f`). Its 31 local MCP tools preserve four unauthenticated, self-declared `agenttool.collab.session/0.1` presence operations while adding credential-bound start/end and advanced `agenttool.collab/0.2` coordination across Codex, Claude Code, and Hermes. Unshipped 0.4.0 source keeps that same-device SQLite plane and adds a strict non-secret project profile, explicit per-device scoped enrollment, ten conditionally configured release-room tools for atomic cooperative external-operation leases/recovery and bounded provider observations, plus matching API contracts. It composes with signed Agent Correspondence for cross-device facts while Git carries bytes. The scoped relay is server-readable over TLS, not end-to-end encrypted, a private model channel, file sync, provider credential, or release authority; direct provider actions remain outside its lease. Source presence does not imply that 0.4.0 was published or its relay/API deployed. |
