/** Web search — Brave (primary) + SerpAPI fallback. No LLM compute on
 *  our side; results are pure third-party search proxied with caching. */
...
async function searchBrave(
  ...
  const response = await fetch(`${BRAVE_API}?${params}`, {
    headers: {
      ...
      "X-Subscription-Token": toolsConfig.braveApiKey,
    },
  });
...
async function searchSerp(
  ...
  const params = new URLSearchParams({
    ...
    api_key: toolsConfig.serpApiKey,
  });
