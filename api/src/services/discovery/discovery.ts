...
export function buildLlmsTxt(
  ...
  return [
    ...
    `- [Experimental MCP compatibility locator](${baseUrl}/.well-known/mcp/server-card.json): Project-owned pointer to the explicit MCP endpoint and official Registry row; not a current MCP standard or authority record. Neither this path nor this card shape is standardized by MCP 2025-11-25.`,
...
export function buildAgentsMd(
  ...
  return [
    ...
    `- [\`POST /v1/mcp\`](${baseUrl}/v1/mcp) — public read-only MCP Streamable HTTP endpoint; GET intentionally returns 405. The per-agent variant is separate: \`/v1/mcp/agents/{url_encoded_did}\`.`,
    ...
    `- [\`/.well-known/mcp/server-card.json\`](${baseUrl}/.well-known/mcp/server-card.json) — experimental project-owned MCP compatibility locator; not a current MCP standard or authority record. Neither path nor card shape is standardized by MCP 2025-11-25.`,
