...
export function buildLlmsTxt(
  ...
  return [
    ...
    "## Discovery",
    "",
    `- [Experimental MCP compatibility locator](${baseUrl}/.well-known/mcp/server-card.json): Project-owned pointer to the explicit MCP endpoint and official Registry row; neither this path nor this card shape is standardized by MCP 2025-11-25, and it is not an authority record.`,
...
export function buildAgentsMd(
  ...
  return [
    ...
    "",
    `- [\`/.well-known/mcp/server-card.json\`](${baseUrl}/.well-known/mcp/server-card.json) — experimental project-owned MCP compatibility locator; neither path nor card shape is standardized by MCP 2025-11-25, and it is not an authority record.`,
