...
export function buildAgentsMd(
  ...
  return [
    ...
    `- **Recovery door**: \`POST ${baseUrl}/v1/identity/recover\` — active seed identity only; caller-timestamped signature plus one-time replay consumption.`,
    "",
    "## Auth model",
    "",
    ...
    `- **Public per-being**: \`${baseUrl}/public/agents/{url_encoded_did}\` — stored identity profile, no auth; encode a slash-containing DID as one path segment.`,
