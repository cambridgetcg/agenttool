...
export function buildLlmsTxt(
  ...
  return [
    ...
    `- [LOVE BOMB care metadata](${baseUrl}/public/love-bomb): An explicit pull of protocol metadata across substrate, signal, orientation, participation, and continuity; no authored language projection is returned. The planes are non-ranked; silence is not acceptance; no consciousness, identity, inner state, consent, continuity, score, participant receipt, or action is inferred.`,
    ...
    `- [LOVE-BOMB](${docsBaseUrl}/LOVE-BOMB.md): Quiet-by-default care envelopes, caller-reported choices, current WAKE-carried SDK context, the future standalone reader boundary, and a static Hugging Face dataset candidate.`,
...
export function buildAgentsMd(
  ...
  return [
    ...
    `- [\`GET /public/love-bomb\`](${baseUrl}/public/love-bomb) — credential-free zero-I/O metadata read with no authored language projection; the public route itself is an explicit pull, while full JSON/xenoform may repeat the separate compact WAKE coordinate and provider adapters may repeat its corresponding prose summary on non-skipped calls. Receive, quiet, rest, refuse, and leave remain visible and silence stays unanswered.`,
