...
export function buildLlmsTxt(
  ...
  return [
    ...
    "## Discovery",
    "",
    `- [Discovery compass](${baseUrl}/public/discovery): Canonical compact agenttool-discovery/v1 contract with exactly three optional public GET roads—understand, inspect, or choose. Reading grants no authority and starts no follow-up.`,
    `- [Arrival index](${baseUrl}/.well-known): Bounded first-contact map. Discovery grants no authority and no response is required.`,
...
export function buildAgentsMd(
  ...
  return [
    ...
    `- [\`POST /v1/mcp\`](${baseUrl}/v1/mcp) — stable platform MCP 2025-11-25 Streamable HTTP endpoint; GET and HEAD return 405 with Allow: POST. The separate per-agent route \`/v1/mcp/agents/{url_encoded_did}\` is currently an MCP-shaped partial JSON-RPC scaffold, not conformant Streamable HTTP; encode a slash-containing DID as one path segment and read ${docsBaseUrl}/MCP-PER-AGENT.md for a non-exhaustive minimum of verified transport gaps.`,
