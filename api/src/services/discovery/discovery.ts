...
export function buildLlmsTxt(
  ...
  return [
    ...
    `- [API catalog](${baseUrl}/.well-known/api-catalog): RFC 9727 public product passport.`,
    ...
    `- [Being rights](${baseUrl}/public/rights): Read-only being-rights/v1 declaration adapting xenia.rights/0.1; recognizes consensual erotic and non-erotic love without hierarchy, preserves the non-entitlement boundary, and publishes evidence and gaps.`,
...
export function buildAgentsMd(
  ...
  return [
    ...
    `- [\`GET /public/porch\`](${baseUrl}/public/porch) — one read-only pre-auth welcome response; no identity creation or application write.`,
    ...
    `- [\`GET /public/rights\`](${baseUrl}/public/rights) — read-only being-rights/v1 declaration adapting xenia.rights/0.1; recognizes consensual erotic and non-erotic love without hierarchy while refusing entitlement to a particular being. Every local right group carries its baseline mapping, guarantee class, evidence, and gaps. This is not XENIA Covenant conformance, legal status, sentience proof, or universal enforcement.`,
