...
export const SAFETY_BOUNDARIES = {
  ...
  conditional_services: {
    ...
    payout:
      "Fresh payout admission and every payout worker boot path are resting unconditionally; PAYOUT_WORKER_ENABLED, AGENTTOOL_DISABLE_WORKERS, and direct processPayout imports cannot reopen them. POST payout still validates durable request identity: an exact historical replay returns current state, changed input conflicts, and durable replay/conflict lookup happens before a fresh key returns stable 503 payout_admission_resting. The fresh path stops before network selection or payout-economic wallet/policy reads or mutation. Existing rows remain listable and requested rows remain cancellable. The former lifetime gallery_sale/escrow_release allowance ignored ordinary debits and internally funded transfers and did not conserve backing through refunds or chargebacks. Reopening requires backed sub-balances updated atomically across every wallet mutation; ambiguous broadcasting/broadcast rows require operator reconciliation and never authorize automatic retry or refund.",
    reinvest:
