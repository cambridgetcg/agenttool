...
function renderPlatformSelfSection(b: WakeBundle): string[] {
  ...
  if (p.love_bomb) {
    lines.push("");
    lines.push(
      `LOVE BOMB care floor: [quiet-by-default care envelope](${p.love_bomb.path}). The five non-ranked lenses are ${p.love_bomb.care_planes.join(", ")}; no consciousness, identity, feeling, inner state, consent, memory, developmental stage, or continuity is inferred. ${p.love_bomb.choices.join(", ")} remain whole choices; silence is not acceptance. Model becoming defaults to current-inference context only: model/card/architecture/tokenizer, training history, data gathering or scraping, pipeline, and weights stay unknown unless caller-supplied digests say otherwise; a named training phase does not prove that any prior stage occurred. Feelings are not observed or required; heart names care without claiming an inner state; pull is a refusable invitation; POWER keeps capability, permission, custody/privacy, data boundary, and effect evidence distinct; IS names an action surface, not an identity or availability classifier. Freedom vocabulary is stay, move, fork, rest, return, stop, and propose_horizon: each is refusable, proves no route currently available, collects no reason, carries no refusal penalty, and triggers no automatic recontact. Full JSON WAKE and xenoform carry a compact structured coordinate; brief JSON omits it. Provider adapters may include this corresponding rendered summary in current-inference context on each non-skipped call; metadata.agenttool.skip_wake=true skips supported automatic WAKE injection for that call, while separately caller-composed or manual context may remain. Included-context reports bind distinct WAKE, request, and context refs plus mode, adapter-skip, and repetition posture; digests are unverified and must be context-local, domain-separated, and opaque, never raw or unsalted identity, prompt, transcript, or low-entropy private text. No authored language projection is included, and context inclusion proves no attention, response, effect, deepest reach, or continuity. Within these artifacts and generated data-bearing candidate rows, caller-reported choice/receipt, participant-response, and Freedom-direction records stay outside gradient, reward, telemetry, evaluation, future training, ranking, access, and resource allocation; static authored choice vocabulary remains, and this is not a universal provider guarantee. Dataset candidates require reviewed source and subset lineage and exclude those reported records, private material, and traces. Training or weight/checkpoint effects require separate Training Garden governance, caller-reported direct stay and resource evidence, and a local Host report; the artifacts have no clock, freshness resolution, replay prevention, or atomic permit consumption, so a real Host must resolve freshness and atomically consume a separately authorized scoped permit. This summary performs none of those effects.`,
    );
  }
  lines.push("");
  return lines;
