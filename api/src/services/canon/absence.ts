...
import { stripComments as realStripComments } from "../../lib/strip-comments";
...
function stripComments(src: string): string {
  // Delegates to the real scanner. The one-line regex this used to be treated
  // a Hono route glob (`"/v1/x/*"`) as an opening block comment and deleted
  // everything to the next `*` + `/` — 810 of index.ts's 1385 lines.
  return realStripComments(src, { blank: true });
}
