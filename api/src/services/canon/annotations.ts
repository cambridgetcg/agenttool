...
/** The annotation shape, kind left open. Kept beside `scanAnnotations`'s
 *  per-kind pattern deliberately: if one changes and the other does not,
 *  `annotationProbeMatchesScanner` in
 *  api/tests/doctrine/annotation-scan-covers-the-repo.test.ts goes red. */
export const ANNOTATION_PROBE =
  /@enforces\s+urn:agenttool:[a-z][a-z0-9-]*\/[a-z][a-z0-9-]+/;
...
export function strayAnnotatedFiles(): StrayAnnotatedFile[] {
  ...
  for (const abs of walk(REPO_ROOT, () => true)) {
    ...
    // The criterion is "would the scanner extract an annotation here", not
    // "does this text contain the marker". A substring test reports a file
    // that only *quotes* the convention — a placeholder such as
    // `@enforces urn:agenttool:commitment/<slug>` inside a comment — and a
    // stray report that cannot be resolved by widening the roots is a
    // report the reader learns to skip. `ANNOTATION_PROBE` is the same
    // shape `scanAnnotations` matches, with the kind left open.
    if (!ANNOTATION_PROBE.test(text)) continue;
