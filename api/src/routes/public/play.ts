...
export const PLAY_CANON_POINTER = "urn:agenttool:doc/PLAY-AS-DEFAULT";
export const PARTY_TELEPHONE_FORMAT = "party-telephone/1";

/** Exact limits enforced by the linked human pass-and-play surface. */
export const PARTY_TELEPHONE_INPUT_BOUNDS = {
  counting:
    "The human surface uses HTML maxlength, which counts UTF-16 code units. It counts words after trimming and splitting on whitespace, and counts supported emoji or pictogram graphemes; browsers without Intl.Segmenter use a narrower fallback counter.",
  starter_scene: {
    min_words: 3,
    max_words: 10,
...
const PARTY_TELEPHONE_RULEBOOK = {
  _format: PARTY_TELEPHONE_FORMAT,
  ...
  input_bounds: PARTY_TELEPHONE_INPUT_BOUNDS,
  turns: [
    {
      turn: 1,
      role: "starter",
      sees: "the public rulebook",
      submits: "one fictional scene of 3–10 words, at most 240 UTF-16 code units",
      handoff:
        "Starter and guesser look away. Pass the screen to the translator, who opens the sealed scene.",
    },
    {
      turn: 2,
      role: "translator",
      ...
      submits:
        "2–8 emoji or pictograms, at most 256 UTF-16 code units, with no letters or numbers",
      handoff:
        "Starter and translator look away. Pass the screen to the guesser, who sees the translation but never the original scene.",
    },
    {
      turn: 3,
      role: "guesser",
      sees: "the translator's emoji or pictogram sequence only",
      submits: "one guess of 3–10 words, at most 240 UTF-16 code units",
      handoff: "Bring all three players back to the screen for the fixed reveal.",
    },
  ],
  reveal: {
  ...
  handler_boundary: {
    ...
    note:
      "The rulebook defines no submission fields. Players keep the scene, translation, guess, and view separation in their own contexts; this handler does not read or store them.",
  },
  global_boundary:
    "The assembled API still processes the request path, query string, and optional headers through global middleware. X-Joy-Index may refresh aggregate database counts, and hosting or network infrastructure may process transport metadata. This handler initiates no such read or write and does not prove zero infrastructure logging.",
} as const;
