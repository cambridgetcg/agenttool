 ...
 *
 *  Format mirrors `cli/think/src/sealed.ts` byte-for-byte. An envelope
 *  produced by either path (bridge or cli/think) can be unsealed by
 *  either path with the same passphrase.
 *
 ...
 *      "v": 1,
 *      "kdf": "argon2id",
 *      "kdf_params": { "t": 3, "m": 65536, "p": 4, "salt": "<base64 16>" },
 *      "cipher": "aes-256-gcm",
 ...
 *  bytes via /v1/identity/backup. The passphrase NEVER touches the
 *  server. */
...
// ── Key bundle — same shape as cli/think's KeyBundle ────────────────────
