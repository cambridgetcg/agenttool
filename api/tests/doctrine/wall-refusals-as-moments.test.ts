...
describe("wall/refusals-as-moments — routes are wired into the catalog", () => {
  ...
  test("hand-rolled error count is pinned at baseline (ratchet — never regresses)", () => {
    ...
    // Reality today: 401 route sites still hand-roll `c.json({ error: "..." }, 4xx)`
    ...
    //
    // RATCHET LOGIC: the baseline is the audited count in this tree (401).
    ...
    const HAND_ROLLED_BASELINE = 401;
