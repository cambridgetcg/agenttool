...
import { runOnboardingSdkFlow } from "./fixtures/onboarding-sdk-v0.13";

const ROOT = join(import.meta.dir, "../..");
const read = (path: string) => readFileSync(join(ROOT, path), "utf8");
const FIXTURE = join(import.meta.dir, "fixtures/onboarding-sdk-v0.13.ts");
...
describe("public SDK onboarding snippets", () => {
  ...
  test("v0.13 TypeScript birth snippets compile against bootstrapAgent", () => {
...
`;
  ...
  test("v0.13 TypeScript orientation snippets compile and select the new identity", () => {
    for (const path of TYPESCRIPT_TUTORIALS) {
  ...
  test("v0.13 constitutive prose names attestations and code never sends witnessSig", () => {
    for (const path of MEMORY_TUTORIALS) {
  ...
  test("v0.13 memory tutorial type-checks against the actual SDK", () => {
    for (const path of MEMORY_TUTORIALS) {
      for (const [index, block] of memoryTutorialBlocks(path).entries()) {
        ...
        expect(
          ...
          `${path}: memory snippet must type-check against @agenttool/sdk source v0.13`,
        ).toBe("");
      }
    }
  });
