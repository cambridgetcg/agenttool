...
const FIXTURE = join(import.meta.dir, "fixtures/onboarding-sdk-v0.16.ts");
...
describe("public SDK onboarding snippets", () => {
  ...
  test("v0.16 TypeScript birth snippets compile against bootstrapAgent", () => {
    for (const path of TYPESCRIPT_TUTORIALS) {
      ...
      expect(
        ...
        `${path}: birth.ts must type-check against @agenttool/sdk source v0.16`,
      ).toBe("");
    }
...
`;
  ...
  test("v0.16 TypeScript orientation snippets compile and select the new identity", () => {
    for (const path of TYPESCRIPT_TUTORIALS) {
      ...
      expect(
        ...
        `${path}: snippet must type-check against @agenttool/sdk source v0.16`,
      ).toBe("");
    }
  }, COMPILER_TEST_TIMEOUT_MS);

  test("v0.16 memory tutorial uses store(content, options)", () => {
  ...
  test("v0.16 constitutive prose names attestations and code never sends witnessSig", () => {
    for (const path of MEMORY_TUTORIALS) {
  ...
  test("v0.16 memory tutorial type-checks against the actual SDK", () => {
    for (const path of MEMORY_TUTORIALS) {
      for (const [index, block] of memoryTutorialBlocks(path).entries()) {
        ...
        expect(
          ...
          `${path}: memory snippet must type-check against @agenttool/sdk source v0.16`,
        ).toBe("");
      }
    }
  }, COMPILER_TEST_TIMEOUT_MS);

  test("the representative v0.16 first-wake flow compiles and executes against mocked fetch", async () => {
