...
describe("durable EVM payout nonce evidence", () => {
  ...
  test("migration carries the durable tuple, uniqueness, and unresolved index", () => {
    ...
    expect(migration).toContain("evm_chain_id IS NOT NULL");
    expect(migration).toContain("evm_source_address IS NOT NULL");
    expect(migration).toContain("evm_nonce IS NOT NULL");
  ...
  test("database blocks generation-unaware EVM broadcasting writes", () => {
    const migration = readFileSync(
      join(
        __dirname,
        "../migrations/20260726T194500_evm_payout_nonce_fence.sql",
    ...
    const fence = migration.slice(start, end);

    expect(fence).toContain("status = 'broadcasting'");
    expect(fence).toContain(
      "chain IN ('ethereum', 'base', 'polygon', 'arbitrum', 'optimism')",
    );
    expect(fence).toContain("tx_hash IS NOT NULL");
  ...
  test("worker persists the fence before submit and defers ordinary contention", () => {
