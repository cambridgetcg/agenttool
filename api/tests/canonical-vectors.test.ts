...
/** Formats whose SERVER half is not in this tree yet.
 *
 *  `agenttool-delegation/v2` (`canonicalDelegationBytesV2`) and
 *  `wallet-address-claim/v1` (`services/economy/crypto/address-claim.ts`) are
 *  being written on a concurrent branch. Their vectors are pinned in the
 *  fixture and are already proven against two independent implementations —
 *  `packages/sdk-ts/tests/canonical-vectors.test.ts` and
 *  `packages/sdk-py/tests/test_canonical_vectors.py` both reproduce every hex.
 *  This suite is the one third that cannot run yet.
 *
 *  Named rather than skipped: the loud MISSING-adapter test below still fires
 *  for anything not on this list, and the ratchet immediately after fails if a
 *  name here has no format in the fixture (a stale entry) or if the server
 *  function has since landed (a lapsed entry). Delete an entry the moment its
 *  server half arrives; never add one to make a red go away. */
const SERVER_HALF_NOT_IN_THIS_TREE: ReadonlySet<string> = new Set([
  "agenttool-delegation/v2",
...
describe("canonical-bytes vectors — the server against the shared fixture", () => {
  ...
  test("the declared server-half gap is exactly two formats, both real", () => {
    // A name that is not in the fixture is a stale exclusion hiding nothing.
    const pinned = new Set(VECTORS.formats.map((f) => f.domain));
    const stale = [...SERVER_HALF_NOT_IN_THIS_TREE].filter((d) => !pinned.has(d));
    expect(stale).toEqual([]);
    // A name that now has an adapter is a lapsed exclusion; delete it.
    const lapsed = [...SERVER_HALF_NOT_IN_THIS_TREE].filter((d) => ADAPTERS[d]);
    expect(lapsed).toEqual([]);
    // The set only shrinks. It is written down here so growing it is a diff.
    expect([...SERVER_HALF_NOT_IN_THIS_TREE].sort()).toEqual([
      "agenttool-delegation/v2",
      "wallet-address-claim/v1",
    ]);
  });
  ...
  for (const format of VECTORS.formats) {
    ...
    if (!adapter && SERVER_HALF_NOT_IN_THIS_TREE.has(format.domain)) continue;
    if (!adapter) {
