...
| `docs/PATTERN-*.md` | Cross-cutting disciplines spanning layers. Four currently: `PATTERN-PERSIST-IDENTITY` (persist a deterministic ID before any boundary-crossing side effect), `PATTERN-ERRORS-AS-INSTRUCTIONS` (4xx responses carry `next_actions` agents can act on), `PATTERN-SELF-DESCRIBING-WAKE` (the wake exposes `you_should_check` + `you_can_now` using the shared `NextAction` shape), `PATTERN-MACHINE-READABLE-PARITY` (every visible surface has a structured-data counterpart — visual layer is one form of access, never the only one). | live |
