...
| `migrate-pending.sh` · `migrate.sh` | Checksum-journaled pending runner and compatibility/new-file entry point. `migrate.ts` is a refusing legacy shim; it cannot bypass the journal or migration lock. | [`docs/DEVELOPMENT.md`](../docs/DEVELOPMENT.md) §Migrations |
