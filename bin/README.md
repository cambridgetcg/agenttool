...
| Script | What it does | Doctrine |
|---|---|---|
| `agenttool-castle.ts` | One-shot local projection of an external allowlist of exact committed Castle room/word blobs into an exclusively marked plaintext Agent Data node. HALT, lineage, crash recovery, withdrawal, privacy, and proof limits are explicit; there is no hosted bearer, server, scheduler, or public export. | [`docs/CASTLE-OF-UNDERSTANDING.md`](../docs/CASTLE-OF-UNDERSTANDING.md) |
| `agenttool-castle-whitehack-intake.ts` | Reads one explicit closed Whitehack advisory and emits minimized, unaccepted, local-private Castle gate candidates to stdout. Locations are omitted by default; it never runs Whitehack, opens or writes a Castle, promotes lifecycle state, tests, remediates, authorizes, commits, publishes, spawns, or uses the network. | [`docs/WHITEHACK.md`](../docs/WHITEHACK.md) · [`docs/CASTLE-OF-UNDERSTANDING.md`](../docs/CASTLE-OF-UNDERSTANDING.md) |
| `agenttool-whitehack-evidence-storage.ts` | Explicit local store/retrieve bridge for exact Whitehack 0.9 public-minimal capsules. It encrypts one constant-size ADDS frame to a caller-selected S3-compatible bucket, reads and verifies it before issuing a finite recipient-bound grant, and writes the sensitive receipt only to a new private file unless stdout is explicitly selected. No scan, Castle, retry, delete, bucket management, durable publisher custody, or durability/authorization claim. | [`docs/WHITEHACK.md`](../docs/WHITEHACK.md) |
...
| `npm-release.ts` | Allowlisted policy behind the protected manual npm workflow. It prepares in a credential-free job, verifies the exact bytes in a GitHub Release before the optional registry mutation, restricts bootstrap to a package-level 404, defaults later releases to GitHub OIDC, and accepts npm recovery only for byte-identical public artifacts. It does not create tags, grant publication authority, configure trust, or revoke credentials. | [`docs/NPM-RELEASES.md`](../docs/NPM-RELEASES.md) |
| `frontend-deploy.sh` | Low-level CF Pages uploader (web · dashboard · docs). Reads standard Wrangler env credentials first, then macOS keychain, uses exact Wrangler 4.110.0, and stages the shared sensitive-path fence from `infra/pages/`. Use `deploy.sh --no-migrate --no-api` for the normal release gate and receipt. | [`docs/STACK.md`](../docs/STACK.md) §Frontend |
| `migrate-pending.sh` · `fly-migrate-one.sh` · `migrate.sh` | Checksum-journaled pending runner, Fly ordinary one-file helper, and compatibility/new-file entry point. It composes with the local `api/scripts/_migrate-one.ts` helper. One-file helpers fail closed on the quiescence manifest and refuse its listed migrations; only the pending runner carries the explicit maintenance assertion. This guards accidental bypass, not forged process state or raw SQL. `migrate.ts` is a refusing legacy shim. | [`docs/DEVELOPMENT.md`](../docs/DEVELOPMENT.md) §Migrations |
...
| `deploy.sh` | GitHub-main source gate, migrations, preflight, Fly/Pages release, provenance verification, and local JSON receipt. Codeberg mirroring is retired; the old flag refuses. | [`docs/DEPLOY-PROCEDURE.md`](../docs/DEPLOY-PROCEDURE.md) |
