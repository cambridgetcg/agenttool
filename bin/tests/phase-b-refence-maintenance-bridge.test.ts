...
describe("closed source normalization", () => {
  ...
  test(
    ...
    async () => {
      ...
      expect(currentFacts.leafSHA256).toBe(
        "035dd500a5aaed195364f292ee7a29b225abc8f952c2d2e8103dd5a357b5e460",
      );
      ...
      expect(currentFacts.emittedLeafSHA256).toBe(
        "51eaa1fcf908e983232432e0d927effcc0bf502e9c5e67a2f1acb6b2c2ee2337",
      );
    },
    15_000,
  );
  ...
  test("bridge normalization zeros only its unique self declaration", () => {
    ...
    expect(normalized).toContain(
      "71fa9f6cd4de141add54500701e56de5e49c74089e27dd13a2be77bb54bbe44d",
    );
  ...
  test("Git reader allowlist covers exact contract proof forms and refuses near misses", () => {
    ...
    const proof = bridge.slice(
      bridge.indexOf("async function readProductionGitProof("),
      bridge.indexOf(
        "function processProofFromBytes(",
        bridge.indexOf(
          "async function readProductionGitProof(",
        ),
      ),
    );
    for (
    ...
    for (
      ...
      const promise = runControllerRolloutCore({
        ...
        dependencies: {
          ...
          proveCordonedRuntime: async () => digest("3"),
  ...
  test("an uncertain provider child never launches recovery", async () => {
    ...
    await expect(runControllerRolloutCore({
      ...
      dependencies: {
        ...
        proveCordonedRuntime: async () => digest("2"),
