...
describe("boring test spine", () => {
  ...
  test("pins required Linux and native macOS gates plus reproducible preparation", async () => {
    ...
    const launchCore = JSON.stringify(jobs["launch-core"]);
    expect(launchCore).toContain("--network none --read-only");
    expect(launchCore).not.toContain("--publish");
    expect(launchCore).not.toContain("secrets.");
    expect(launchCore).toContain("target=$GITHUB_WORKSPACE,readonly");
    expect(launchCore).toContain("target=/usr/local/bin/bun,readonly");
    expect(launchCore).toContain("--host 127.0.0.1 --port 56268");
    expect(launchCore).toContain("--maintenance-quiesced");
    expect(launchCore).toContain("launch-core-journey.test.ts");
    const fixtureDockerfile = await readFile(join(ROOT, "tests/launch-postgres/Dockerfile"), "utf8");
    expect(fixtureDockerfile).toContain('"listen_addresses=127.0.0.1"');
    expect(fixtureDockerfile).toContain('"port=56268"');
    expect(fixtureDockerfile).toContain('"cron.launch_active_jobs=off"');
...
`,
  ...
  test("keeps software npm publication manual, exact-artifact, and protected", async () => {
    ...
    expect(publishWorkflows).toEqual(["publish-gospel.yml", "publish-npm.yml", "publish-pypi.yml"]);
