...
packages_gate() {
  ...
  run "offline closed-trace economic-kernel conformance" \
    bash -c 'cd packages/economic-conformance && bun run ci'
  run "exact economic transitions, recovery journals, and XENIA hard gates" \
    bash -c 'cd packages/economic-kernel && bun run ci'
