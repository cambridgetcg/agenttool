...
import { z } from "zod";
import { verifyInvocationCompletion } from "../api/src/services/marketplace/sig";
import {
  canonicalSettlementReceiptBytes, outputDigestHex, verifySettlementReceipt,
  type SettlementReceiptCore,
} from "../api/src/services/marketplace/settlement-receipt-verify";
import { parseWitnessEntries } from "../api/src/services/marketplace/witness";
import {
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  if (argv.length === 1 && argv[0] === "--help") {
    process.stdout.write("usage: bun bin/agenttool-zerone-reconcile.ts --input <path|-> [--format json|text]\nOffline only; docs/SETTLEMENT-RECEIPTS.md defines the bounded schema and evidence limits.\n");
    return 0;
  }
