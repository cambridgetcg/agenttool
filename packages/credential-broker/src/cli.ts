...
function parseConfig(value: unknown): BrokerConfig {
  ...
  for (const [alias, raw] of Object.entries(value.credentials)) {
    if (
      ...
      !isRecord(raw.auth) ||
      typeof raw.auth.kind !== "string" ||
      !["bearer", "header"].includes(raw.auth.kind) ||
      (raw.account !== undefined && typeof raw.account !== "string") ||
      (raw.auth.headerName !== undefined && typeof raw.auth.headerName !== "string") ||
      (raw.auth.prefix !== undefined && typeof raw.auth.prefix !== "string")
    ) {
