...
function inspectConsentDecision(value: unknown): {
  ...
  try {
    ...
    if (decision.allowed === true) {
      return { allowed: true, reasonCode: "consent_denied" };
    }
    const reasonCode = decision.reasonCode;
    return {
      allowed: false,
      reasonCode:
        typeof reasonCode === "string" && SAFE_CONSENT_REASON_CODE.test(reasonCode)
          ? reasonCode
          : "consent_denied",
    };
  } catch {
    // A hostile getter/proxy is malformed consent output, never a diagnostic.
    return { allowed: false, reasonCode: "consent_denied" };
  }
}
...
export class BrokerServer {
  ...
  async #record(event: AuditEvent): Promise<void> {
    ...
    try {
      ...
      // Multiple records may already be awaiting the sink. The first catch
      // synchronously owns the latch and notification; later catches observe
      // the failed state and do nothing.
      if (!this.#auditHealthy) return;
  ...
  async #accept(socket: Socket): Promise<void> {
    ...
    const activeDecoder = new FrameDecoder((value) => {
  ...
  async #dispatch(state: SessionState, request: WireRequest): Promise<WireResponse> {
    try {
      ...
      if (request.type === "grant.request") {
        ...
        const decision = inspectConsentDecision(rawDecision);
        if (!decision.allowed) {
          await this.#record({
            auditId: newAuditId(),
            at: this.#clock.wallNow().toISOString(),
