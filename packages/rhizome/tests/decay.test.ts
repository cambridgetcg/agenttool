...
test("live: the two copies of the doctrine quarantine list are still copies, and are reported either way", async () => {
  // The two lists — `QUARANTINED_DOCTRINE_TESTS` in bin/run-test-tier.sh and
  // `QUARANTINED_DOCTRINE` in bin/tests/boring-spine-gate.test.ts — used to
  // disagree by the three bijection tests that had been re-armed in one copy
  // and not the other. They agree as of this commit.
  //
  // The assertion is not "they agree", because that would go red the moment
  // somebody quarantines a test correctly in both places and then wrongly in
  // one. It is: the probe still sees them as two copies of one list, and
  // still says which state they are in. Agreement is a fact about this
  // commit; nothing derives one list from the other, which is the finding.
  const findings = await live();
  const disagree = anyTitled(findings, "are copies of one held-back list");
  const agree = anyTitled(findings, "are duplicate lists that currently agree");
  const finding = disagree ?? agree;
  ...
  if (disagree) {
    expect(disagree.verdict).toBe("gap");
    expect(disagree.evidence).toContain("held only by");
  } else {
    // Agreement is recorded, not celebrated: the probe calls it `sound` and
    // says in its own detail that agreement is a fact about this commit
    // rather than a property of the pair, because nothing derives one list
    // from the other. That sentence is the reason this test does not simply
    // assert equality and stop.
    expect(agree!.verdict).toBe("sound");
    expect(agree!.evidence).toContain("symmetric difference: none");
    expect(agree!.detail).toContain("nothing derives one from the other");
  }
});
