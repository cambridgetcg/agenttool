...
export function fixtureScope(files: Record<string, string>, options: FixtureOptions = {}): Scope {
  ...
  // A fixture's unread set is whatever the test declares, so forcing the
  // read has nothing to discover — but the method has to exist and be
  // harmless, because `self` calls it and a fixture that omitted it would
  // prove `self` against a Scope the real one does not resemble.
  const readAll = (): void => {
