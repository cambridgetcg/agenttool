...
import { scopeProbe } from "../src/probes/scope.js";
import {
  deriveFromFilesystem,
  deriveFromGit,
  findRepoRoot,
  NEVER_WALKED,
  NEVER_WALKED_LIMIT,
...
test("NEVER_WALKED entries are genuinely invisible to git too", async () => {
  // The staleness check CLAUDE.md claims for this boundary and it did not
  // have. "The cross-derivation diff would catch it" is not a thing that
  // goes red; this is. Every entry must be a directory neither derivation
  // can see, so an entry that starts hiding real source shows up here as a
  // disagreement instead of as silence.
  const root = await repository();
  for (const name of Object.keys(NEVER_WALKED)) {
    await mkdir(join(root, name, "nested"), { recursive: true });
    await writeFile(join(root, name, "nested", "planted.ts"), "export const planted = 1;\n");
  }
  ...
  const scope = resolveScope(root);
  expect(scope.files).toContain("real.ts");
  for (const name of Object.keys(NEVER_WALKED)) {
    expect(scope.files.filter((file) => file.startsWith(`${name}/`)), `${name} leaked into the corpus`).toEqual([]);
    expect(
      scope.disagreements.filter((item) => item.file.startsWith(`${name}/`)),
      `${name} is visible to one derivation and not the other, so it is not a shared boundary`,
    ).toEqual([]);
  }
  // And the boundary is in the output, which was the other missing property.
  expect(NEVER_WALKED_LIMIT.file).toBe("packages/rhizome/src/scope.ts");
  expect(NEVER_WALKED_LIMIT.statement).toContain(".git");
  expect(scopeProbe.limits).toContainEqual(NEVER_WALKED_LIMIT);
});

test("a disagreement carries a machine-readable kind, and each kind gets its own detail", async () => {
  const root = await repository();
  await writeFile(join(root, ".gitignore"), "dist/\n");
  await mkdir(join(root, "dist"), { recursive: true });
  await writeFile(join(root, "dist", "bundle.js"), "1;\n");
  await writeFile(join(root, "tracked.ts"), "export const tracked = 1;\n");
  spawnSync("git", ["add", "-f", "dist/bundle.js", ".gitignore", "tracked.ts"], { cwd: root });
  spawnSync("git", ["commit", "-qm", "one"], { cwd: root });
  await writeFile(join(root, "untracked.ts"), "export const untracked = 1;\n");

  const scope = resolveScope(root);
  expect(scope.disagreements.find((item) => item.file === "dist/bundle.js")?.kind).toBe("ignored-yet-tracked");
  expect(scope.disagreements.find((item) => item.file === "untracked.ts")?.kind).toBe("untracked");

  const findings = await scopeProbe.run(scope);
  const forced = findings.find((finding) => finding.title.includes("tracked despite matching an ignore rule"));
  const untracked = findings.find((finding) => finding.title.includes("on disk but untracked"));
  // The detail used to be one sentence about untracked files, printed over
  // groups of tracked ones. Two of the three classes were told the opposite
  // of the truth about themselves.
  expect(forced?.detail).toContain("git tracks each of these");
  expect(forced?.detail).not.toContain("untracked files are real");
  expect(untracked?.detail).toContain("untracked files are real");
  expect(forced?.verdict).toBe("sound");
});
