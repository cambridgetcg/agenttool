...
re-deriving that judgement. On the current run 80 of 311 findings are `SOUND`.
...
On this repository at the time of writing: 2,742 tracked, 2,801 walked, 2,853
in the union, 163 explained disagreements — 111 untracked files a git-based
...
these numbers; a sibling session is writing into this tree as they are read.
...
Each disagreement now carries a machine-readable `kind` — `untracked`,
`symlink`, `ignored-yet-tracked`, `unexplained` — and each kind gets its own
sentence. Consumers used to reconstruct the kind by prefix-matching the English
reading, and the probe printed one explanation about untracked files over
*every* group: two of the three tracked classes were told the opposite of the
...
asserts), never by name. Two checks answered by reading — whether a guard's own
detector regex covers every way the language binds the name it defends, and
whether a fixture-driven suite has any floor under its case count — and one
answered only by running, behind `RHIZOME_MUTATE=1`: whether the guard dies
when the guarded property is actually broken.

There used to be a third static check, *has anyone shown this detector
firing?*, and it was a list of English phrasings matched against the guard's
whole file text, comments included. It was wrong in both directions: a scanner
with no negative control at all read as `SOUND` because a comment in it
happened to say "this scan can still fail when the checkout is shallow", and a
real negative control worded outside the list read as a `GAP`. Neither error
made any test red — reverting the list to match everything, or nothing, left
the probe's own suite at 14 pass / 0 fail either way. A check that cannot do
what its name says, sitting inside the probe that exists to name exactly that,
is not a check.

It is now the `plant-detector-match` mutant operator: a line this corpus
already contains, which the guard's own detector matches, is copied into a file
the guard reads, and the guard is run. Firing is observed. A run that produces
no pass/fail counts at all is reported `LIMIT` — inconclusive — rather than as
the guard firing, because a shadow that stopped building is not evidence.
Nothing is claimed by default: without `RHIZOME_MUTATE=1` the probe publishes
the controls it derived and did not execute.
...
packages a mutant touches are copied and edited; the guard runs there. The set
of directories a copy skips is derived from the repository's own `.gitignore`
files rather than typed — it was a literal list of six names, inside the
package whose first run reported six copies of one list elsewhere — and the two
that are symlinked back instead of copied (`node_modules`, `.venv`) are
exported, reasoned, checked against the derived set by a test, and published as
a declared limit. Nothing inside the checkout is written, and a guard that is
not green *unmutated* is skipped by name rather than counted either way.
Without the variable the probe publishes the mutation plan it did not run.

If the harness's own control mutant survives — the one every loader in this
tree explicitly refuses — the caveat is carried **on each affected finding**,
not only in a separate section. It used to be a section, which meant a page of
survival verdicts printed above it at full confidence, each stating that a
guard is green while the property it claims is broken. A reader who stopped
after the first page read conclusions the run had already invalidated.
...
end of its file, is itself a `GAP`.

`rhizome --probe self` is the command a reader uses to ask *where can rhizome
not see?*, and for most of this probe's life it answered "nowhere". Not because
rhizome could see everything: `Scope.unread` fills in as files are read, so a
run of `self` alone — which reads nothing — reported an empty unread set and a
header saying `0 unread`, while a full run of the same corpus reported 43. The
probe now forces the corpus read before publishing, so the number is a property
of the tree rather than of the caller's `--probe` flags, and a corpus with
nothing unread says so out loud rather than by omission.
...
- **41 files in the corpus are never read.** Binary, over the 2MB ceiling, or
...
- **Registration is checked by filename**, and only for direct children of
  `src/probes/` whose name ends in `.ts`. A probe at `src/probes/<name>/index.ts`
  is not checked at all. The prefix-matching version of this check reported
  seven helper modules of three registered probes as unregistered probes, which
  is the package's own pathology; narrowing it moved the miss rather than
  removing it. The `.ts` half of the boundary was undeclared until it was
  written down here, and it no longer drops silently: any other direct child of
  the probes directory is published as a `LIMIT` naming the file.

**Three literal boundaries rhizome keeps, each asserted rather than silent** —
exported, reasoned where it is declared, staleness-checked by a test that goes
red, and published in rhizome's own output as a declared limit:

| boundary | where | checked by |
|---|---|---|
| `NEVER_WALKED` — the directories the walk never enters | `src/scope.ts` | `tests/scope.test.ts` builds a repository containing each entry and fails if `git ls-files` can see it |
| the recogniser vocabularies — what "reads / sweeps / asserts the tree" looks like in source | `src/recognisers.ts` | `tests/recognisers.test.ts` pins exactly which spellings have no witness in this corpus, in both directions, and fails if a probe declares a private copy |
| `SCOPE_NAME` / `ALLOWLIST_NAME` — what a repository calls its scope and exemption constants | `src/probes/edge.ts` | `tests/edge.test.ts` pins the unwitnessed alternatives and asserts the vocabulary still recognises the names it was written for |

`PROSE_EXTENSIONS` was a fourth, and it was the one that mattered: three
private copies of it lived in `probes/edge.ts`, `probes/reach.ts` and
`probes/decay/reach.ts`, **one of them strictly narrower than the other two**
for no stated reason. On its first run over this repository rhizome reported
the six-fold `encodePathSegment` duplication in the SDKs while reproducing that
exact defect inside itself — nothing went red when the copies diverged, because
nothing compared them. There is now one definition, in `src/prose.ts`, and a
test that fails when a second one appears.

**Per probe, in the probe's own words** — `bun run rhizome --probe self`
prints all 49 with their `file:line`:
...
| `pretend` | four mutant operators, aimed rather than blanket; the survival rate is a fact about these operators, not about the guards, in-file tables are never shrunk live, and whether a guard's detector fires is unanswered unless `RHIZOME_MUTATE=1` |
...
produces 65 findings anchored inside it: **0 gaps**, 10 `SOUND`, 55 `LIMIT`.
