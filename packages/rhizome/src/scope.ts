...
import type { DisagreementKind, ProbeLimit, Scope, ScopeDerivation, ScopeDisagreement } from "./types.js";
...
/** `NEVER_WALKED`'s own declaration, published by the `scope` probe.
 *
 *  CLAUDE.md claims four properties for this boundary — exported, reasoned
 *  in place, staleness-checked by something that goes red, and named in
 *  rhizome's own output. Two of them were true. The cross-derivation diff
 *  was described as the staleness check, but a diff is not a thing that
 *  goes red, and no probe ever printed the word `NEVER_WALKED`, so the
 *  boundary lived only in a comment inside the file that holds it — which
 *  is the shape of an unstated boundary in the tool that hunts them.
 *
 *  `tests/scope.test.ts` now fails when an entry here is not also invisible
 *  to `git ls-files`, and this limit puts the boundary in the report. */
export const NEVER_WALKED_LIMIT: ProbeLimit = Object.freeze({
  statement: `the filesystem walk never enters ${Object.keys(NEVER_WALKED)
    .map((name) => `${name}/`)
    .join(", ")}, so nothing inside is in the corpus for any probe`,
  why: `${Object.entries(NEVER_WALKED)
    .map(([name, reason]) => `${name}: ${reason}`)
    .join("; ")}. Asserted rather than derived, and checked from outside: tests/scope.test.ts builds a repository containing each entry and fails if git ls-files can see it, so an entry that is not genuinely invisible to both derivations cannot stay in this list quietly`,
...
/** One sentence per kind. Exhaustive by type: a new `DisagreementKind`
 *  does not compile until it has a reading, so the two cannot drift. */
export const READINGS: Readonly<Record<DisagreementKind, string>> = Object.freeze({
  untracked: "on disk but untracked: a scanner that enumerates via git cannot see this file",
  symlink:
  ...
  "ignored-yet-tracked": "tracked despite matching an ignore rule: a walk that honours .gitignore cannot see a force-added file",
  unexplained: "tracked but not produced by the walk: rhizome's own ignore matching dropped a file git keeps",
});
...
function diff(root: string, derivations: readonly ScopeDerivation[]): ScopeDisagreement[] {
  ...
  for (const [file, present] of membership) {
    ...
    // The kind is the classification; the reading is prose FOR the kind.
    // They were one string, and every consumer had to re-derive the kind by
    // matching a prefix of the sentence — which is how the `scope` probe
    // came to print an explanation of untracked files over a group of
    // tracked ones.
    const kind: DisagreementKind = absentFrom.includes("git-tracked") ? "untracked" : explainAbsence(root, file).kind;
    out.push({
      file,
      presentIn: ids.filter((id) => present.has(id)),
      absentFrom,
      kind,
      reading: READINGS[kind],
    });
  }
...
export function resolveScope(root?: string): Scope {
  ...
  const read = (file: string): string | null => {
    ...
    // Only a path the corpus actually holds can be one the corpus could not
    // read. `self` resolves every probe's limit anchor through `read`, and
    // an anchor pointing outside this repository was landing in the unread
    // set — so a run against another root reported eighteen unreadable files
    // that were not in its corpus at all.
    if (text === null && inCorpus.has(file)) unread.push(file);
  ...
  // `unread` accumulates as files are read, so before anything has read the
  // tree it is empty — and "empty" reads as "rhizome can see everything",
  // which is the opposite of what it means. `readAll` makes the set a
  // property of the corpus instead of a property of which probes ran.
  const readAll = (): void => {
