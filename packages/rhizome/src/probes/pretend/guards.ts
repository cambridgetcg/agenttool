...
import { ASSERTS, READS_THE_TREE } from "../../recognisers.js";
...
export function generatedFiles(scope: Scope): Set<string> {
  ...
  for (const disagreement of scope.disagreements) {
    if (disagreement.kind === "ignored-yet-tracked") out.add(disagreement.file);
  }
  return out;
}
...
/** Read the regular expressions a file declares as named bindings.
 *
 ...
 *
 *  The name filter was `[A-Z][A-Z0-9_]{2,}` — SCREAMING_CASE only, in both
 *  dialects. That is a house style, not a language rule, and it is exactly
 *  the kind of undeclared narrowing this probe reports in other people's
 *  code: a guard that writes `const localDefinition = /…/` had no detector
 *  at all as far as `pretend` was concerned, so it was neither measured nor
 *  named — it produced no output, which reads as nothing to report. Any
 *  named binding of a regular expression counts now.
 *
 *  What remains outside, declared as a probe limit rather than left
 *  implicit: a pattern that is never given a name (`if (/…/.test(x))`), one
 *  built by `new RegExp(...)` from strings at run time, and one that is a
 *  property of an object or class rather than a standalone binding. Those
 *  are patterns whose text this reader cannot recover without executing the
 *  file, and rhizome does not execute what it reads. */
export function literalPatterns(lines: readonly string[]): LiteralPattern[] {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const tsHead = /^[ \t]*(?:export[ \t]+)?(?:const|let|var)[ \t]+([A-Za-z_$][\w$]*)[ \t]*(?::[^=]*)?=[ \t]*(.*)$/.exec(line);
    if (tsHead !== null) {
      let tail = tsHead[2] ?? "";
      let cursor = i;
    ...
    const pyHead = /^[ \t]*([A-Za-z_][\w]*)[ \t]*=[ \t]*re\.compile\(/.exec(line);
    if (pyHead !== null) {
      let body = "";
      let depth = 0;
...
export const DECLARES_CASES = /\btest(?:\.each|\.skip|\.only|\.todo|\.failing)?\s*\(|\bit\s*\(|\bdescribe\s*\(|parametrize/;

/** A case declaration that OPENS a block, as opposed to one already inside
 *  a case. `describe` is deliberately absent: a loop inside a `describe`
 *  body still declares one case per entry. */
const OPENS_A_CASE = /\btest\s*\(|\bit\s*\(|^\s*def\s+test_/;

/** Does the loop at `index` generate one case per entry, or does it run
 *  inside a single case that was already declared?
 *
 *  The distinction is the whole claim. `for (const row of TABLE) expect(…)`
 *  inside one `test(...)` makes a table of ASSERTIONS: deleting entries
 *  removes assertions from a test that still runs, and the suite's case
 *  count does not move. `for (const row of TABLE) test(…)` makes a table of
 *  CASES: deleting entries makes the suite shorter and nothing red. Only
 *  the second is what this probe reports, and looking eight lines ahead for
 *  the word `test(` could not tell them apart — it matched the enclosing
 *  `test(` line of the first shape as readily as the generated `test(` of
 *  the second, and reported a table of assertions as a table of cases.
 *
 *  Answered structurally rather than by pattern. TypeScript: track brace
 *  depth from the top of the file and remember the depth at which each
 *  still-open `test(`/`it(` began; if any is still open at the loop, the
 *  loop is inside a case. Python: the same question is indentation — a
 *  `def test_…` whose body is still open at the loop's indentation. */
export function generatesCases(lines: readonly string[], index: number): boolean {
  let depth = 0;
  let indentOfOpenTest: number | null = null;
  const openTestDepths: number[] = [];

  // Comments are stripped over the whole text, not line by line: a header
  // block containing a brace would otherwise unbalance every depth below
  // it, and this repository's guards all open with one.
  const stripped = stripComments(lines.slice(0, index).join("\n")).split("\n");

  for (let i = 0; i < index; i += 1) {
    const raw = lines[i] ?? "";
    const code = codeOnly(stripped[i] ?? "");
    const indent = raw.search(/\S/);

    // Python: a test function's body is everything indented past its `def`.
    if (indentOfOpenTest !== null && indent !== -1 && indent <= indentOfOpenTest) indentOfOpenTest = null;
    if (/^\s*def\s+test_/.test(code)) indentOfOpenTest = indent === -1 ? 0 : indent;

    const opensCase = OPENS_A_CASE.test(code);
    const depthBefore = depth;
    for (const char of code) {
      if (char === "{" || char === "(") depth += 1;
      else if (char === "}" || char === ")") {
        depth -= 1;
        while (openTestDepths.length > 0 && depth <= openTestDepths[openTestDepths.length - 1]!) openTestDepths.pop();
      }
    }
    // A `test("name", () => {` that closes on its own line declares a case
    // and does not enclose anything below it.
    if (opensCase && depth > depthBefore) openTestDepths.push(depthBefore);
  }

  const loopIndent = (lines[index] ?? "").search(/\S/);
  const insidePythonTest = indentOfOpenTest !== null && loopIndent > indentOfOpenTest;
  return openTestDepths.length === 0 && !insidePythonTest;
}
...
export function inFileTables(scope: Scope, guard: Guard): CaseCollection[] {
  ...
  for (const collection of literalCollections(lines, 400)) {
    ...
    for (let i = 0; i < lines.length && !declares; i += 1) {
      ...
      if (!generatesCases(lines, i)) continue;
