/** rhizome/probes/pretend/mutants — the operators, and why there are only four.
 *
 ...
 *                       exactly as before.
 *
 *    plant-detector-match
 *                       a guard that sweeps the tree with a detector claims
 *                       its verdict depends on that detector. Plant, into a
 *                       file the guard reads, a line copied verbatim out of
 *                       this corpus that the guard's own detector matches,
 *                       and run the guard. This replaces a check that used
 *                       to answer the same question by looking for the
 *                       English phrase "can still fail" in the guard's
 *                       file — which called a scanner with no negative
 *                       control sound because a comment in it contained the
 *                       words, and called a real negative control a gap
 *                       because it was worded differently. A question about
 *                       whether something fires is answered by making it
 *                       fire.
 *
 ...
 *  is a declared probe limit: these four are the operators, not the
 *  operators worth having.
 */

import type { Scope } from "../../types.js";
import type { CaseCollection } from "./guards.js";
import { owningPackage, type Edit } from "./harness.js";
...
export const OPERATORS: readonly MutantOperator[] = Object.freeze([
  ...
  {
    id: "plant-detector-match",
    property: "a guard's verdict actually depends on its own detector firing",
    expectation: "die",
  },
]);
...
/** Everything the plant operator needs, all of it read out of the tree. */
export interface DetectorPlant {
  /** The guard whose detector is under test. */
  guardFile: string;
  /** The detector, as the guard declares it. */
  patternName: string;
  patternLine: number;
  /** Where in the corpus the planted line was copied from. */
  witnessFile: string;
  witnessLine: number;
  /** The line itself, verbatim. Never synthesised: a line this package
   *  invented would be a line no compiler has ever accepted. */
  text: string;
  /** A file the guard reads, which does not currently match the detector. */
  target: string;
}

/** Plant a real line the guard's own detector matches into a file the guard
 *  reads, so the guard can be watched either firing or not firing.
 *
 *  The line is copied verbatim out of this repository rather than generated
 *  from the pattern. Generating a string that satisfies a regular
 *  expression is easy; generating one that is also valid TypeScript or
 *  Python is not, and a mutant that stops the guard compiling teaches
 *  nothing — the guard goes red for a reason that has nothing to do with
 *  its detector. Copying a line that already exists in this corpus keeps
 *  the planted text something a real toolchain has already accepted. The
 *  residual risk — a line valid where it came from and not where it lands —
 *  is not assumed away: `pretend.ts` reads a run that produced no pass/fail
 *  counts as inconclusive rather than as the guard firing. */
export function plantDetectorMatch(scope: Scope, plant: DetectorPlant): Mutant | null {
  const text = scope.read(plant.target);
  if (text === null) return null;
  const body = text.endsWith("\n") ? text : `${text}\n`;
  const owner = owningPackage(scope, plant.target)?.directory;
  const guardOwner = owningPackage(scope, plant.guardFile)?.directory;
  return {
    operator: "plant-detector-match",
    description:
      `${plant.target} gains one line that ${plant.patternName} matches, copied verbatim from ${plant.witnessFile}:${plant.witnessLine}`,
    file: plant.guardFile,
    line: plant.patternLine,
    edits: [{ file: plant.target, text: `${body}${plant.text}\n` }],
    copies: [...new Set([owner, guardOwner].filter((entry): entry is string => entry !== undefined))],
    expectation: "die",
  };
}
