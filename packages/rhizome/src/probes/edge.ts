...
import { isProse, PROSE_EXTENSION_LIST } from "../prose.js";
 ...
 *
 *  A naming vocabulary, and one of the two literal boundaries this probe
 *  keeps. It cannot be derived — what a repository calls its scope
 *  constants is a fact about English and about house style, and deriving it
 *  from the tree would only ever produce the names somebody already used.
 *  Exported so it can be checked from outside, staleness-checked by
 *  `tests/edge.test.ts` (every member must name at least one real
 *  collection in this corpus, so a dead alternative goes red), and
 *  published as a `ProbeLimit` below: a collection whose name announces a
 *  scope in some other vocabulary is only classified when the file it lives
 *  in also walks the tree. */
export const SCOPE_NAME = /(SCAN|ROOT|ROOTS|DIR|DIRS|PATH|PATHS|SOURCE|SOURCES|TREE|TREES|TARGET|TARGETS|AREA|AREAS)$/i;

/** Names that announce "these are the ones I let through".
 *
 *  The same kind of boundary as `SCOPE_NAME`, with the same four
 *  properties, and a sharper failure: an allowlist this vocabulary does not
 *  recognise is not checked for stale entries at all, and produces no
 *  output — which reads as an allowlist with nothing wrong with it. */
export const ALLOWLIST_NAME =
  /(ALLOW|EXEMPT|IGNORE|SKIP|EXCLUD|EXCLUS|KNOWN|BASELINE|WHITELIST|LEGACY|GRANDFATHER|WAIVER|SUPPRESS)/i;

/** Every alternative in the two naming vocabularies, for the staleness
 *  check in `tests/edge.test.ts`. */
export const NAME_VOCABULARIES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  SCOPE_NAME: Object.freeze((/\(([^)]*)\)/.exec(SCOPE_NAME.source)?.[1] ?? "").split("|")),
  ALLOWLIST_NAME: Object.freeze((/\(([^)]*)\)/.exec(ALLOWLIST_NAME.source)?.[1] ?? "").split("|")),
});

/** Alternatives above that name no literal collection in this repository
 *  today, pinned exactly rather than assumed.
 *
 *  A naming vocabulary is allowed to contain a word nobody here has used —
 *  that is the point of it, since the next hard-coded scope will be named
 *  by somebody who has not read this file. What is not allowed is for the
 *  set to be unknown: `tests/edge.test.ts` compares the unwitnessed
 *  alternatives against this list and fails on any difference in either
 *  direction, so a word that starts being used has to be removed from here,
 *  and a word that quietly stops matching appears here rather than nowhere.
 *  Without it, "the vocabulary is fine" and "the vocabulary matches nothing
 *  any more" are the same observation. */
export const NAME_ALTERNATIVES_WITHOUT_WITNESS: Readonly<Record<string, readonly string[]>> = Object.freeze({
  SCOPE_NAME: Object.freeze(["SCAN", "DIR", "TREES", "TARGET", "AREA", "AREAS"]),
  ALLOWLIST_NAME: Object.freeze(["EXCLUS", "WHITELIST", "GRANDFATHER", "WAIVER"]),
});
...
const LIMITS: readonly ProbeLimit[] = [
  ...
  {
    statement:
      "a collection is classified as a scope only when its name matches SCOPE_NAME or its file walks the tree, and as an allowlist only when its name matches ALLOWLIST_NAME; a boundary named in some other vocabulary is not classified at all and produces no output",
    why: "what a repository calls its scope and exemption constants is a fact about English and house style, not something derivable from this corpus — a derived list would hold only the names somebody already used, which is the boundary that cannot be seen from inside. Both vocabularies are therefore asserted: exported from src/probes/edge.ts, reasoned at the declaration, and staleness-checked by tests/edge.test.ts, which pins exactly which alternatives have no witness in the tree",
  ...
  {
    statement: `the member-set check skips ${PROSE_EXTENSION_LIST}, so a documentation tree that lists nine of twenty-three packages is not reported`,
    why: "a stale sentence is a documentation concern rather than a guarantee wearing the wrong shape, and flagging every doc that names four packages is severity theatre. The extension list is not this probe's own: it is the single definition in src/prose.ts, which three probes used to carry three private copies of — one of them strictly narrower, which is the defect this package exists to report",
