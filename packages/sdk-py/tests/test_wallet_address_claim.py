...
The digests below are the wire contract. ``api/tests/wallet-address-claim.test.ts``
and ``packages/sdk-ts/tests/wallet-address-claim.test.ts`` pin the identical
values. If any of the three drift, an agent signing here cannot register an
address and the failure surfaces only as an opaque ``claim_signature_invalid``.
