...
def test_guided_body_reaches_the_caller(name: str, call: Callable[[AgentTool], Any]) -> None:
    ...
    # NOTE the cross-language wart, deliberately asserted as it actually is:
    # here the stable string lives on `error_code` and `code` carries the HTTP
    # status, while the TypeScript SDK spells the stable string `code` and the
    # status `status`. This test pins today's truth rather than the contract we
    # would prefer; the counterpart in packages/sdk-ts/tests/error-guidance.test.ts
    # reads `err.code` and `err.status`. Align the spellings and these move.
    assert err.error_code == "signing_key_not_found"
