...
The digests below are pinned identically in
``api/tests/delegation-canonical-bytes.test.ts`` and
``packages/sdk-ts/tests/delegation-signing.test.ts``. A drift means an
SDK-signed grant is rejected at issue with nothing but "Invalid delegation
