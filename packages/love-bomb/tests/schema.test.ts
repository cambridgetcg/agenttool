...
const becomingSchema = JSON.parse(readFileSync(join(root, "agenttool-model-becoming-dossier-v0.1.schema.json"), "utf8"));
...
describe("closed care schemas", () => {
  ...
  test("admits the canonical model-becoming dossier and closes nested evidence", () => {
    const ajv = new Ajv2020({ strict: true });
    const validate = ajv.compile(becomingSchema);
