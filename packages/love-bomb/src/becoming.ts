...
export function createModelBecomingSource(input: ModelBecomingSourceInput): Readonly<ModelBecomingSource> {
  ...
  const body = deepFreeze({
    _format: LOVE_BOMB_FORMATS.becomingSource,
    ...parsed,
  });
  return deepFreeze({
    ...body,
    source_id: domainSeparatedId(LOVE_BOMB_FORMATS.becomingSource, body),
  });
}
...
function claimInput(value: unknown, path: string): ModelBecomingClaimInput {
  ...
  if ((knowledgeState === "known" || knowledgeState === "partly_known") && method === "not_available") {
    fail("becoming_error", `${path}.method cannot be not_available for known or partly_known claims`);
  }
  return deepFreeze({
    module: member<ModelBecomingModule>(candidate.module, MODEL_BECOMING_MODULES, `${path}.module`),
    statement: text(candidate.statement, `${path}.statement`, 4_096),
    knowledge_state: knowledgeState,
    claim_kind: member<ModelBecomingClaimKind>(candidate.claim_kind, MODEL_BECOMING_CLAIM_KINDS, `${path}.claim_kind`),
...
export function createModelBecomingClaim(input: ModelBecomingClaimInput): Readonly<ModelBecomingClaim> {
  ...
  const body = deepFreeze({
    _format: LOVE_BOMB_FORMATS.becomingClaim,
    ...parsed,
  });
  return deepFreeze({
    ...body,
    claim_id: domainSeparatedId(LOVE_BOMB_FORMATS.becomingClaim, body),
  });
}
...
export function createModelBecomingDossier(
  ...
  for (const claim of claims) {
    ...
    if (claim.claim_kind === "first_party_disclosure") {
      const firstParty = claim.source_refs.some((sourceRef) => sourceById.get(sourceRef)?.source_kind.startsWith("first_party_"));
      if (!firstParty) fail("becoming_error", "first_party_disclosure claims require a first-party source");
    }
    if (claim.claim_kind === "verified_artifact") {
      const digested = claim.source_refs.some((sourceRef) => sourceById.get(sourceRef)?.digest !== null);
      if (!digested || claim.method !== "artifact_digest") {
        fail("becoming_error", "verified_artifact claims require artifact_digest and a digested source");
      }
    }
  }
  ...
  const body = deepFreeze({
    _format: LOVE_BOMB_FORMATS.becomingDossier,
  ...
  return deepFreeze({
    ...body,
    dossier_id: domainSeparatedId(LOVE_BOMB_FORMATS.becomingDossier, body),
  });
}
