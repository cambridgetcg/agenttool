...
- Native `fetch` + native `AbortSignal.timeout` for general HTTP; Math Cards,
  LOVE BOMB, and Attention Lab use dedicated direct `undici` package dispatchers so Bun
  startup proxy credentials cannot enter these no-auth transports
...
package.json                — Package config (v0.23.0, ESM)
...
## Dependencies
- **Runtime**: `@noble/ed25519 ^2.2.3`, `@noble/hashes ^2.0.1` (Phase 5+ crypto only — matches api server + cli/think versions for byte-identical wire format), plus `undici ^7.29.0` solely for the Math Cards, LOVE BOMB, and Attention Lab direct no-env-proxy dispatchers. Other HTTP, AES-256-GCM, and abort signals use platform-native APIs.
- **Dev**: `typescript ^5.7`, `@types/bun ^1.2`
- **API**: Authenticated hosted calls go to `https://api.agenttool.dev` (configurable via `baseUrl`); `at.kingdomFramework`, `at.mathCards`, `at.attentionLab`, and standalone `LoveBombClient` / `AttentionLabClient` use separate credential-free requests; pure `WakeContinuityLayer` / `at.wakeContinuity` performs no request; `at.data` and `at.kingdomOS` are separate local authorities
- **Auth**: Reads `AT_API_KEY`, accepts `apiKey`, or accepts a mutually
  ...
  The public KINGDOM framework, Math Cards, LOVE BOMB, and Attention Lab clients receive
...
- `package.json` — Package metadata (v0.23.0, ESM)
