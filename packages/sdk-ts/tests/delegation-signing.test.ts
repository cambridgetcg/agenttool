 ...
 *  reproduce byte for byte. The digests below come from the shared fixture
 *  `docs/specs/canonical-bytes-vectors.json` (`agenttool-delegation/v2`) and
 *  are pinned identically in `packages/sdk-py/tests/test_delegation_signing.py`.
 *  The SERVER half (`canonicalDelegationBytesV2` and
 *  `api/tests/delegation-canonical-bytes.test.ts`) is being written on a
 *  concurrent branch and is not in this tree; the fixture is what the three
 *  implementations agree against. */
