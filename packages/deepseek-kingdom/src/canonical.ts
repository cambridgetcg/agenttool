...
// A maximally populated proposal admits 64 candidates, each with 64 claim
// references, plus the complete 64-claim source binding. Keep that shape
// constructible while bounding hostile input before protocol validation.
...
function inspectJson(root: unknown, captureOriginals = false): InspectedJson {
  ...
  function accountUtf8(value: string, path: string): void {
    // Every valid UTF-8 encoding is at least as many bytes as the source has
    // UTF-16 code units. Reject obvious oversize input before scanning it.
...
function inspectArray(
  ...
  if (keys.length !== length + 1) {
    // Preserve the specific small-shape diagnostic without scanning an
    // already-oversized key list merely to classify its extras.
...
function inspectObject(
  ...
  try {
    // ECMAScript exposes no paged own-key iterator. Capture this one list to
    // count the caller's existing shape, then avoid a descriptor-map copy.
  ...
  for (let index = 0; index < entries.length; index += 1) {
    ...
    const inspected = visit(entryValue, depth + 1, `${path}.${key}`);
