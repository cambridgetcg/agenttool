...
const treasures = [
  ...
  },
  {
    ...
    signal: "A dedicated ties subset tests whether reward models invent margins between answers that are deliberately equally acceptable.",
    use: "Represent AnswerEquivalenceClass and margin_should_not_exceed so equivalent answers remain unranked.",
    observations: [
    ...
    reason_codes: ["equivalent_answers_not_ranked", "human_prompt_privacy_review", "structure_only"],
    consequence: "requires_rights_review",
    node_kinds: ["DatasetRelease", "TrainingPhase", "RiskGate", "AnswerEquivalenceClass"],
    allowed_relations: ["derived_from", "used_in_phase", "measures", "acceptable_variant", "invalid_variant", "margin_should_not_exceed", "requires_review"],
  },
  {
    rank: 17,
...
writeFileSync(`${spaceRoot}/source-manifest.json`, prettyJsonBytes({
  ...
  dataset_revision_status: "injected_after_initial_dataset_publish",
