...
describe("package boundary", () => {
  test("is zero-runtime-dependency, side-effect-free, and has no CLI", () => {
    expect(packageJson.name).toBe("@agenttool/living-substrate");
  ...
  test("exports only library contracts, schemas, and declaration hint", () => {
    ...
    for (const target of Object.values(packageJson.exports).flatMap(
      (entry: any) =>
  ...
  test("keeps source imports and ambient capabilities inside the pure boundary", () => {
    ...
    expect(sources).not.toMatch(
      /\b(?:getRandomValues|randomBytes|randomFill|randomInt|randomUUID|webcrypto)\b/u,
    );
  });

  test("ships two parseable closed Draft 2020-12 schemas", () => {
    for (const file of [
      "agenttool-living-substrate-map-v0.1.schema.json",
      "agenttool-regeneration-proposal-v0.1.schema.json",
    ]) {
  ...
  test("keeps the KINGDOM declaration zero-effect and unregistered", () => {
    ...
    expect(extension).toMatchObject({
      ...
      id: "living-substrate",
      package: "@agenttool/living-substrate",
      version: "0.1.0-dev.0",
    ...
    expect(extension.capabilities).toContain("living-substrate:map-create");
    expect(extension.capabilities).toContain(
      "living-substrate:regeneration-proposal-create",
    );
    expect(extension.notes.join(" ")).toMatch(
      /declaration-only.*not an installed KINGDOM host contract/i,
    );
    expect(extension.notes.join(" ")).toMatch(
      /rest.*fallow.*doing nothing.*refusal.*leaving/i,
    );
    expect(extension.notes.join(" ")).toMatch(
      /does not observe.*diagnose.*score.*rank/i,
    );
  });
});
