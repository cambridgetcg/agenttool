...
const HARDENED_PREVIEW_SCHEMA_VERSION = 2;
...
const COORDINATION_SESSION_COLUMNS = [
  "id",
  "workspace_id",
  "worktree_id",
  "actor",
...
interface CoordinationSessionRow {
  id: string;
  workspace_id: string;
  worktree_id: string;
  actor: string;
...
export class CollabStore {
  ...
  private initialize(): void {
    ...
    const migrate = this.db.transaction(() => {
      ...
      if (versionAfterLock < HARDENED_PREVIEW_SCHEMA_VERSION) {
        this.migrateToV2(legacyPreparation);
      } else if (versionAfterLock === HARDENED_PREVIEW_SCHEMA_VERSION) {
        this.migrateHardenedPreviewToV3();
      }
      this.validateV2Shape("coordination_sessions", [
        HARDENED_PREVIEW_SCHEMA_VERSION,
        SCHEMA_VERSION,
      ]);
  ...
  private preflightDatabase(): void {
    ...
    if (this.tableNames().length > 0) {
      this.validateLegacyOrV2Shape(userVersion);
      if (userVersion >= HARDENED_PREVIEW_SCHEMA_VERSION) {
        this.validateAllJournals();
      }
    }
  }
  ...
  private prepareLegacyMigration(): PreparedLegacyWorkspace[] {
    if (
      ...
      || pragmaNumber(this.db, "user_version") >= HARDENED_PREVIEW_SCHEMA_VERSION
    ) return [];
    ...
    assertDistinctLegacyWorktreeIdentities(prepared);
    return prepared;
  }
  ...
  private migrateToV2(preparedWorkspaces: PreparedLegacyWorkspace[]): void {
    ...
    this.db.exec(`
      ...
      CREATE TABLE coordination_sessions (
        ...
        actor TEXT NOT NULL,
        role TEXT,
        parent_session_id TEXT REFERENCES coordination_sessions(id),
      ...
      CREATE INDEX coordination_sessions_workspace_status_idx
        ON coordination_sessions(workspace_id, status, last_seen_at);

      CREATE TABLE reports (
        id TEXT PRIMARY KEY,
        workspace_id TEXT NOT NULL REFERENCES workspaces(id),
        task_id TEXT,
        from_session_id TEXT REFERENCES coordination_sessions(id),
        from_actor TEXT NOT NULL,
        to_session_id TEXT REFERENCES coordination_sessions(id),
      ...
      CREATE TABLE task_reviews (
        ...
        reviewer_session_id TEXT NOT NULL REFERENCES coordination_sessions(id),
        created_at TEXT NOT NULL,
        event_sequence INTEGER NOT NULL,
      ...
      CREATE TABLE task_recoveries (
        ...
        recovered_by_session_id TEXT NOT NULL REFERENCES coordination_sessions(id),
    ...
    const insertMigration = this.db.query(`
    ...
    insertMigration.run(SCHEMA_VERSION, COLLAB_COORDINATION_PROTOCOL, now);
    this.migrationFailpoint?.("v2_verified");
  }

  private migrateHardenedPreviewToV3(): void {
    this.validateV2Shape("sessions", [HARDENED_PREVIEW_SCHEMA_VERSION]);
    const journalHeads = this.db.query(`
      SELECT id, epoch_id, event_head_sequence, event_head_hash
      FROM workspaces ORDER BY id
    `).all();
    ...
    this.db.exec(`
      ALTER TABLE sessions RENAME TO coordination_sessions;
      DROP INDEX sessions_workspace_status_idx;
      CREATE INDEX coordination_sessions_workspace_status_idx
        ON coordination_sessions(workspace_id, status, last_seen_at);
      ...
      CREATE INDEX sessions_workspace_presence_idx
        ON sessions(workspace_id, left_at, presence_expires_at);
    `);
    this.migrationFailpoint?.("v3_session_planes_created");

    const migratedRows = this.db.query(`
      SELECT * FROM coordination_sessions ORDER BY id
    `).all();
    if (canonicalJson(migratedRows) !== canonicalJson(sessionRows)) {
      throw new CollabError(
        "migration_session_rows_changed",
        "Credential-bound preview session rows changed during the v0.3 table rename",
      );
    }
    const headsAfter = this.db.query(`
      SELECT id, epoch_id, event_head_sequence, event_head_hash
    ...
    this.db.query(`
      INSERT INTO schema_migrations (version, protocol, applied_at)
      VALUES (?, ?, ?)
    `).run(SCHEMA_VERSION, COLLAB_COORDINATION_PROTOCOL, this.timestamp());
    ...
    this.migrationFailpoint?.("v3_verified");
  }
  ...
  private hasColumn(table: string, column: string): boolean {
    return this.tableColumns(table).includes(column);
  }

  private tableColumns(table: string): string[] {
    const rows = this.db.query(`PRAGMA table_info(${quoteIdentifier(table)})`).all() as Array<{
      name: string;
    }>;
    return rows.map((row) => row.name);
  }

  private hasExactColumns(table: string, columns: readonly string[]): boolean {
    return canonicalJson(this.tableColumns(table)) === canonicalJson(columns);
  }
  ...
  private validateLegacyOrV2Shape(userVersion: number): void {
    ...
    const hasV2Marker =
      names.has("coordination_sessions")
    ...
    if (userVersion < HARDENED_PREVIEW_SCHEMA_VERSION) {
      if (
        hasV2Marker
        || pragmaNumber(this.db, "application_id") !== 0
        || (
          names.has("sessions")
          && !this.hasExactColumns("sessions", PRESENCE_SESSION_COLUMNS)
        )
      ) {
        throw new CollabError(
          "database_partial_migration",
          "The collaboration database has unrecognized hardened objects without a committed schema version",
        );
      }
      return;
    }
    if (userVersion === HARDENED_PREVIEW_SCHEMA_VERSION) {
      if (names.has("coordination_sessions")) {
        throw new CollabError(
          "database_partial_migration",
          "The hardened preview database has an unexpected coordination-session table name",
        );
      }
      this.validateV2Shape("sessions", [HARDENED_PREVIEW_SCHEMA_VERSION]);
      return;
  ...
  private validateV2Shape(
    sessionTable: "sessions" | "coordination_sessions",
    ...
    const required: Record<string, string[]> = {
      ...
      [sessionTable]: [...COORDINATION_SESSION_COLUMNS],
      reports: [
    ...
    this.requireColumns(required, "The hardened collaboration schema is incomplete");
    if (!this.hasExactColumns(sessionTable, COORDINATION_SESSION_COLUMNS)) {
      throw new CollabError(
        "database_schema_mismatch",
        "The credential-bound coordination session table has an unexpected shape",
      );
    }
    if (
      sessionTable === "coordination_sessions"
      && !this.hasExactColumns("sessions", PRESENCE_SESSION_COLUMNS)
    ) {
      throw new CollabError(
        "database_schema_mismatch",
        "The self-declared presence session table has an unexpected shape",
      );
    }
    ...
    if (
      canonicalJson(migrationRows.map((row) => row.version))
        !== canonicalJson(expectedMigrationVersions)
      || migrationRows.some(
        (row) => row.protocol !== COLLAB_COORDINATION_PROTOCOL,
      )
    ) {
      throw new CollabError(
        "database_migration_receipt_invalid",
        "The collaboration schema migration ledger is missing or inconsistent",
      );
    }
  ...
  joinSession(input: JoinSessionInput): CollabSession {
    const normalized = {
      ...
      declared_capabilities: normalizeSessionCapabilities(
    ...
    const transaction = this.db.transaction(() => {
      const existing = this.readPresenceSessionByClient(
        normalized.workspace_id,
        normalized.client_instance_id,
      );
      if (existing) {
        requireMatchingPresenceSessionJoin(existing, normalized);
        if (existing.left_at) {
          throw new CollabError(
            "session_instance_ended",
            "This presence session already left; use a new client_instance_id",
      ...
      return presenceSessionFromRow(
        this.requirePresenceSessionRow(id),
        now,
      );
    });
  ...
  getSession(sessionId: string): CollabSession {
    return presenceSessionFromRow(
      this.requirePresenceSessionRow(validateId(sessionId, "session_id")),
      this.timestamp(),
    );
  }
  ...
  listSessions(
    ...
    if (presence === "live") {
      ...
      rows = this.db.query(`
        ...
        ORDER BY joined_at DESC, id DESC LIMIT ?
      `).all(id, boundedLimit) as PresenceSessionRow[];
    } else {
      rows = this.db.query(`
        SELECT * FROM sessions
        WHERE workspace_id = ?
    ...
    return rows.map((row) => presenceSessionFromRow(row, observedAt));
  }
  ...
  heartbeatSession(input: HeartbeatSessionInput): CollabSession {
    ...
    const initial = this.requirePresenceSessionRow(normalized.session_id);
    return this.mutate(
      ...
      normalized,
      () => {
        const row = this.requirePresenceSessionRow(normalized.session_id);
        requirePresenceSessionVersion(row, normalized.expected_version);
        if (row.left_at) {
          throw new CollabError(
            "session_left",
            "An explicitly left presence session cannot heartbeat",
            { session_id: row.id },
          );
        }
        const now = this.timestamp();
        ...
        return presenceSessionFromRow(
          this.requirePresenceSessionRow(row.id),
          now,
        );
      },
    );
  ...
  startSession(input: {
    ...
    if (partitions.length > 1) {
      throw new CollabError(
        ...
        "This repository has multiple preserved v0.1 journals; explicit reconciliation is required before credential-bound sessions can start",
    ...
    const joined = this.db.transaction(() => {
      if (parentSessionId) {
        const parent = this.requireCoordinationSessionRow(parentSessionId);
      ...
      return this.sessionHandle(
        this.requireCoordinationSessionRow(id),
        token,
  ...
  resumeSession(credential: {
    ...
    const resumed = this.db.transaction(() => {
      ...
      const next = this.requireCoordinationSessionRow(row.id);
  ...
  endSession(input: {
    ...
    const ended = this.db.transaction(() => {
      ...
      return coordinationSessionFromRow(this.requireCoordinationSessionRow(row.id));
    }).immediate();
    this.tightenFileModes();
    return ended;
  }

  listCoordinationSessions(workspaceId: string, status?: "active" | "ended"): CoordinationSession[] {
    ...
    const rows = status
      ? this.db.query(`
        ...
        `).all(workspace.id, status) as CoordinationSessionRow[]
      : this.db.query(`
          SELECT * FROM coordination_sessions
          WHERE workspace_id = ?
          ORDER BY joined_at, id
        `).all(workspace.id) as CoordinationSessionRow[];
    return rows.map(coordinationSessionFromRow);
  }
  ...
  acknowledgeSessionCursor(input: {
    ...
    const acknowledged = this.db.transaction(() => {
      ...
      if (anchor.sequence === row.cursor_sequence) {
        ...
        return coordinationSessionFromRow(row);
      }
  ...
  resetSessionCursor(input: {
    ...
    return coordinationSessionFromRow(current);
  }
  ...
  workspaceStatus(workspaceId: string): WorkspaceStatus {
    ...
    return {
      ...
      active_sessions: this.listCoordinationSessions(workspaceId, "active"),
  ...
  offerHandoffForSession(input: {
    ...
    return this.mutateAsSession(
      ...
      (session) => {
        const target = this.requireCoordinationSessionRow(toSessionId);
  ...
  nextForSession(input: {
    ...
    const read = this.db.transaction(() => {
      ...
      return {
        actor: current.actor,
        session: coordinationSessionFromRow(current),
        own_claims: tasks.filter(
          (task) =>
  ...
  private mutateAsSession<T>(
    ...
    operation: string,
    request: unknown,
    apply: (session: CoordinationSessionRow) => T,
  ...
  private authenticateResumeToken(sessionId: string, token: string): CoordinationSessionRow {
    const row = this.db.query(`SELECT * FROM coordination_sessions WHERE id = ?`)
      .get(sessionId) as CoordinationSessionRow | null;
  ...
  private captureRegisteredWorktree(
    session: Pick<CoordinationSessionRow, "id" | "workspace_id">,
  ...
  private requirePreparedWorktree(
    session: CoordinationSessionRow,
  ...
  private sessionHandle(
    ...
    return {
      ...
      session: coordinationSessionFromRow(row),
      credential: {
        session_id: row.id,
        session_token: token,
  ...
  private validateReportInput(
    ...
    if (input.to_session_id) {
      const targetSession = this.requireCoordinationSessionRow(input.to_session_id);
  ...
  private appendEvent(
    ...
    const event: CollabEvent = {
      protocol: COLLAB_COORDINATION_PROTOCOL,
  ...
  private readPresenceSessionByClient(
    workspaceId: string,
    clientInstanceId: string,
  ): PresenceSessionRow | null {
    return this.db.query(`
      SELECT * FROM sessions
      WHERE workspace_id = ? AND client_instance_id = ?
    `).get(workspaceId, clientInstanceId) as PresenceSessionRow | null;
  }

  private requirePresenceSessionRow(sessionId: string): PresenceSessionRow {
    const row = this.db.query(`SELECT * FROM sessions WHERE id = ?`)
      .get(sessionId) as PresenceSessionRow | null;
    if (!row) {
      throw new CollabError(
        "session_not_found",
        `Presence session '${sessionId}' was not found`,
      );
    }
    return row;
  }

  private requireCoordinationSessionRow(sessionId: string): CoordinationSessionRow {
    const row = this.db.query(`SELECT * FROM coordination_sessions WHERE id = ?`)
      .get(sessionId) as CoordinationSessionRow | null;
...
function presenceSessionFromRow(
  row: PresenceSessionRow,
  observedAt: string,
): CollabSession {
  const presence: SessionPresence = row.left_at
    ? "left"
    : observedAt < row.presence_expires_at
      ? "live"
      : "stale";
  return {
...
function requirePresenceSessionVersion(
  row: PresenceSessionRow,
  ...
  if (row.version !== expectedVersion) {
    throw new CollabError(
      ...
      "Presence session version changed",
      {
        session_id: row.id,
        expected_version: expectedVersion,
...
function requireMatchingPresenceSessionJoin(
  row: PresenceSessionRow,
  input: {
    actor_label: string;
    runtime_kind: string;
...
function coordinationSessionFromRow(row: CoordinationSessionRow): CoordinationSession {
  return {
    id: row.id,
    workspace_id: row.workspace_id,
    worktree_id: row.worktree_id,
...
function assertDistinctLegacyWorktreeIdentities(
  workspaces: PreparedLegacyWorkspace[],
): void {
  const selectors = [
    {
      field: "worktrees.root_path",
      value: (workspace: PreparedLegacyWorkspace) => workspace.identity.root_path,
    },
    {
      field: "worktrees.id",
      value: (workspace: PreparedLegacyWorkspace) =>
        workspace.identity.checkpoint.worktree_id,
    },
    {
      field: "worktrees.fingerprint",
      value: (workspace: PreparedLegacyWorkspace) =>
        workspace.identity.worktree_fingerprint,
    },
  ] as const;
  const collisions: Array<{ field: string; workspace_ids: string[] }> = [];

  for (const selector of selectors) {
    const groups = new Map<string, string[]>();
    for (const workspace of workspaces) {
      const value = selector.value(workspace);
      const workspaceIds = groups.get(value) ?? [];
      workspaceIds.push(workspace.id);
      groups.set(value, workspaceIds);
    }
    for (const workspaceIds of groups.values()) {
      if (workspaceIds.length < 2) continue;
      collisions.push({
        field: selector.field,
        workspace_ids: workspaceIds.sort(),
      });
    }
  }
  ...
  collisions.sort((left, right) => {
    const fieldOrder = left.field < right.field ? -1 : left.field > right.field ? 1 : 0;
    if (fieldOrder !== 0) return fieldOrder;
    const leftIds = left.workspace_ids.join("\0");
    const rightIds = right.workspace_ids.join("\0");
    return leftIds < rightIds ? -1 : leftIds > rightIds ? 1 : 0;
  });
  const workspaceIds = [
    ...new Set(collisions.flatMap((collision) => collision.workspace_ids)),
  ].sort();
  throw new CollabError(
    "migration_identity_collision",
    "Legacy workspaces resolve to the same worktree identity; migration will not merge distinct journals",
    {
