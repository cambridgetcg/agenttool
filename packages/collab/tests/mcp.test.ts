...
class StdioMcpHarness {
  ...
  async initialize(): Promise<any> {
    const result = await this.request("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "agenttool-collab-test", version: "0.2.0" },
    });
...
describe("MCP surface", () => {
  ...
  test("states the multi-session, privacy, review, and host boundaries", () => {
    ...
    expect(tools.collab_session_join.description).toContain("never returned in tool output");
  ...
  test("rejects session-only options in legacy mode and workspace changes after binding", async () => {
    ...
    const joined = await callTool(boundNode.server, "collab_session_join", {
  ...
  test("does not expose an absolute credential path through MCP errors", async () => {
    ...
    const result = await callTool(server, "collab_session_join", {
      root_path: root,
  ...
  test("coordinates reported completion, distinct review, and explicit cursor acknowledgement across handlers", async () => {
    ...
    const firstJoin = await callTool(firstServer, "collab_session_join", {
      root_path: root,
      actor: "implementer",
    });
    const secondJoin = await callTool(secondServer, "collab_session_join", {
      root_path: root,
      actor: "reviewer",
    });
  ...
  test("shares repository, worktree, session, and review state across two bundled stdio processes", async () => {
    ...
    expect(implementerInit.serverInfo.version).toBe("0.2.0");
    expect(reviewerInit.serverInfo.version).toBe("0.2.0");

    const implementerJoin = await implementer.callTool("collab_session_join", {
      root_path: root,
      actor: "stdio-implementer",
    });
    const reviewerJoin = await reviewer.callTool("collab_session_join", {
