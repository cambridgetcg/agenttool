...
- **Pages fence**: `pages/` is staged into all three frontend roots by `bin/frontend-deploy.sh`; every request traverses the minimal advanced-mode Worker, which denies canonical sensitive roots and forwards accepted public-asset requests intact to Pages assets
