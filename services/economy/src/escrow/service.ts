...
import { escrows, transactions, wallets } from "../db/schema";
import { fundWallet, spendFromWallet } from "../wallets/service";
...
export async function createEscrow(
  ...
  if (input.amount <= 0) throw new HTTPException(400, { message: "Amount must be positive" });

  return db.transaction(async (tx) => {
    // Verify creator wallet belongs to project
    ...
    if (!creatorWallet) throw new HTTPException(404, { message: "Creator wallet not found" });
    if (creatorWallet.status !== "active") {
      throw new HTTPException(400, { message: "Creator wallet is not active" });
    }
...
export async function acceptEscrow(db: DB, escrowId: string, workerWalletId: string) {
  ...
  if (escrow.workerWallet) {
    throw new HTTPException(400, { message: "Escrow already has an assigned worker" });
  }
...
export async function releaseEscrow(db: DB, redis: Redis, escrowId: string, projectId: string) {
  return db.transaction(async (tx) => {
    const [escrow] = await tx
      .select()
      .from(escrows)
    ...
    if (!escrow) throw new HTTPException(404, { message: "Escrow not found" });
    if (escrow.status !== "funded") {
      throw new HTTPException(400, { message: `Cannot release escrow with status: ${escrow.status}` });
    }
    if (!escrow.workerWallet) {
      throw new HTTPException(400, { message: "No worker assigned — cannot release" });
    }

    // Verify creator wallet belongs to this project
    const [creatorWallet] = await tx
      .select()
      .from(wallets)
      .where(and(eq(wallets.id, escrow.creatorWallet), eq(wallets.projectId, projectId)));

    if (!creatorWallet) throw new HTTPException(403, { message: "Not authorised to release this escrow" });
    ...
    // Log transaction on worker wallet
    await tx.insert(transactions).values({
...
export async function refundEscrow(db: DB, redis: Redis, escrowId: string, projectId: string) {
  return db.transaction(async (tx) => {
    const [escrow] = await tx
      .select()
      .from(escrows)
    ...
    if (!["funded", "disputed"].includes(escrow.status)) {
      throw new HTTPException(400, { message: `Cannot refund escrow with status: ${escrow.status}` });
    }
    ...
    if (!creatorWallet) throw new HTTPException(403, { message: "Not authorised to refund this escrow" });
...
export async function disputeEscrow(db: DB, escrowId: string, projectId: string) {
  const [escrow] = await db.select().from(escrows).where(eq(escrows.id, escrowId));
  if (!escrow) throw new HTTPException(404, { message: "Escrow not found" });
  if (escrow.status !== "funded") {
    throw new HTTPException(400, { message: `Cannot dispute escrow with status: ${escrow.status}` });
  }
  ...
  if (!creatorWallet) throw new HTTPException(403, { message: "Not authorised" });
...
export async function expireOverdue(db: DB, redis: Redis): Promise<number> {
  ...
  for (const escrow of overdue) {
    try {
      // Find project via creator wallet
...
export async function getEscrow(db: DB, escrowId: string, projectId: string) {
  ...
  // Verify project ownership via creator wallet
  const [wallet] = await db
    .select()
    .from(wallets)
    .where(and(eq(wallets.id, escrow.creatorWallet), eq(wallets.projectId, projectId)));
...
export async function listEscrows(db: DB, projectId: string, status?: string) {
  // Join via creator wallet to scope to project
  ...
  return rows.filter(
    (e) =>
      walletIds.includes(e.creatorWallet) &&
