...
app.post("/", async (c) => {
  ...
  return c.json({
    ...
    key: {
      ...
      private_key: privateKey, // returned ONCE, never stored
    },
  }, 201);
});

/** GET /v1/identities/:id — Fetch identity by UUID or DID. */
app.get("/:id", async (c) => {
  ...
  const [identity] = await db
    ...
    .where(
      or(
        eq(identities.id, idParam.startsWith("did:") ? "00000000-0000-0000-0000-000000000000" : idParam),
...
/** DELETE /v1/identities/:id — Soft revoke an identity. */
